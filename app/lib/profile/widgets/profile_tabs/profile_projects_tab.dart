...
class ProfileProjectsTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return switch (projectsAsync) {
      AsyncValue(:final value?) => _ProfileProjectsLoadedSlivers(
        handle: handle,
        posts: value.items,
      ...
      AsyncError(:final error) => _ProfileProjectsErrorSliver(
        error: error,
...
class _ProfileProjectsLoadedSlivers extends ConsumerWidget {
  const _ProfileProjectsLoadedSlivers({
    required this.handle,
    required this.posts,
    ...
    return SliverMainAxisGroup(
      slivers: [
        if (posts.isEmpty)
          SliverFillRemaining(
            hasScrollBody: false,
            child: Center(child: Text(l10n.profileEmptyProjects)),
          )
        else
          SliverList.builder(
              ...
              if (hasMore &&
                ...
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  if (context.mounted) {
                    unawaited(
                      ref
                          .read(userProjectsProvider(handle).notifier)
                          .loadMore(),
                    );
                  }
                });
...
class _ProfileProjectsErrorSliver extends StatelessWidget {
  const _ProfileProjectsErrorSliver({
    required this.error,
