...
func TestInstagramSuggestionEligibilityPolicyRequiresEveryBoundary(t *testing.T) {
	...
	eligible := EligibilitySnapshot{
		...
		Discoverable:          true,
		ConflictFree:          true,
		ImportDirection:       DirectionFollowing,
	...
	cases := []struct {
		...
		{name: "followers direction", mutate: func(s *EligibilitySnapshot) { s.ImportDirection = DirectionFollower }, reason: EligibilityDirection},
