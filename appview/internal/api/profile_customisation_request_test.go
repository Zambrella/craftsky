...
func TestProfileCustomisationRequestAcceptsCompleteSupportedValue(t *testing.T) {
	...
	got, err := api.DecodeProfileCustomisationPut(strings.NewReader(
		`{"colour":"ink","profileBackground":"cubedark"}`,
	))
	if err != nil {
...
func TestProfileCustomisationRequestRejectsInvalidBodies(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			body:      `{"colour":123,"profileBackground":"none"}`,
			wantCode:  "invalid_request",
			wantField: "colour",
		},
		{
			name:      "nested background resource",
			body:      `{"colour":"cobalt","profileBackground":{"asset":"https://example.com/tile.png"}}`,
			wantCode:  "invalid_request",
			wantField: "profileBackground",
		},
		{
			name:      "arbitrary hex colour",
			body:      `{"colour":"#ff00ff","profileBackground":"none"}`,
			wantCode:  "validation_failed",
			wantField: "colour",
		},
		{
			name:      "URL colour",
			body:      `{"colour":"https://example.com/colour","profileBackground":"none"}`,
			wantCode:  "validation_failed",
			wantField: "colour",
		},
		{
			name:      "unsupported background",
			body:      `{"colour":"cobalt","profileBackground":"future-texture"}`,
			wantCode:  "validation_failed",
			wantField: "profileBackground",
		},
		{
		...
		},
		{
			...
			body:      `{"colour":"cobalt","colour":"teal","profileBackground":"none"}`,
			wantCode:  "malformed_body",
			wantField: "_",
		},
		{
		...
		{
			name:      "oversized body",
			body:      `{"colour":"cobalt","profileBackground":"` + strings.Repeat("x", 4096) + `"}`,
			wantCode:  "malformed_body",
			wantField: "_",
		},
	}
