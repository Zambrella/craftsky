...
func (s *FirebaseSender) Send(ctx context.Context, request SendRequest) (ProviderResult, error) {
	...
	message := &messaging.Message{Token: request.Token, Notification: &messaging.Notification{Title: payload.Title, Body: payload.Body}, Data: payload.Data, Android: &messaging.AndroidConfig{TTL: &request.TTL}, APNS: &messaging.APNSConfig{Headers: map[string]string{"apns-expiration": strconv.FormatInt(deadline.Unix(), 10)}}}
