...
Feature: Process-local route-class rate limiting
  Scenario Outline: Requests exceeding either token or device bucket are throttled early
    ...
    And auth class requests are modeled as pre-auth requests with no Craftsky session token
...
Feature: Production CORS policy
  Scenario Outline: Origin handling follows exact allow-list policy
    ...
    And bearer-token Authorization remains an allowed request header for permitted origins
    And cookie credential CORS via Access-Control-Allow-Credentials is not enabled for v1
...
| UT-005 | BR-003, FR-010, FR-011, RULE-003, FR-014 | AC-009, AC-010, AC-017 | Verify CORS exact allow-list, wildcard dev behavior, supported headers/methods, bearer-token `Authorization` header support, no cookie credential CORS, and preflight short-circuit. | Origins: app, marketing, preview, evil, localhost/wildcard dev; preflight headers. | Only allowed origins echoed; preflight succeeds with `Authorization`, `Content-Type`, and `X-Craftsky-Device-Id` request headers allowed; `Access-Control-Allow-Credentials` is not enabled; next/rate limiter not called. | `appview/internal/middleware/cors_test.go` |
...
| UT-007 | BR-002, FR-007, FR-008, FR-009, RULE-006, RULE-007 | AC-007, AC-008, AC-019, AC-020 | Verify limiter keys and counters for token/device route classes, including auth-class pre-auth requests with no Craftsky token, no IP keys, Retry-After calculation, upload attempt counting. | Low limits, fake clock, token/device combinations, auth requests with token absent, failed upload attempts. | Correct key throttled; auth class uses device bucket without requiring token; 429 envelope; Retry-After present; no quota headers; no IP key used. | `appview/internal/middleware/rate_limit_test.go` |
...
| IT-005 | BR-002, FR-007, FR-008, FR-009, RULE-006, RULE-007 | AC-007, AC-008, AC-019, AC-020 | End-to-end limiter behavior across token/device buckets, with auth/login modeled as pre-auth per-device limiting. | Low test limits, fake clock, mux with handler counter. | Exceed read/write/search/upload limits by token and device; exceed auth/login limit with no Craftsky token and a valid device ID. | 429 envelope, Retry-After, no quota headers; handler counter stops; auth class does not require token; upload failures counted. | `appview/internal/middleware/rate_limit_test.go`, route integration tests |
...
| IT-007 | BR-003, FR-010, FR-011, RULE-003 | AC-009, AC-010 | CORS policy with production/dev config. | Load prod config with `https://app.craftsky.social`; dev config with wildcard/localhost. | Send actual and preflight browser requests. | Exact prod origin allowed; marketing/preview/evil not allowed; supported headers include bearer-token Authorization, Content-Type, X-Craftsky-Device-Id; cookie credential CORS is not enabled. | `appview/internal/middleware/cors_test.go`, `appview/internal/app/config_test.go` |
...
| IT-009 | NFR-003, RULE-008 | AC-014, AC-021 | Deployment/config guidance for process-local limiter and defaults. | Load config and inspect the concrete implementation artifact chosen during planning, such as config docs, deployment notes, or startup/operator-facing guidance. | Omit limit envs; supply invalid envs; inspect the chosen deployment guidance surface. | Defaults match requirements; invalid config fails; single-instance/shared-storage warning exists in the chosen artifact. | `appview/internal/app/config_test.go`, docs/config test if available |
...
| MAN-002 | RULE-006, RULE-008 | Deployment constraint and no-IP-key review. | Review the concrete guidance surface selected during planning, such as config docs, deployment notes, startup log text, or operator-facing config validation output. | AppView v1 limiter is documented as process-local/single-instance; multi-instance requires shared limiter or edge enforcement; no AppView limiter key uses client IP. |
...
- Document-review clarifications applied: auth/login limiter tests should model pre-auth requests without requiring a Craftsky token; CORS wording distinguishes bearer-token `Authorization` headers from cookie credential CORS; AC-021 should be satisfied through a concrete planning-selected guidance surface such as config docs, deployment notes, startup log text, or operator-facing validation output.
