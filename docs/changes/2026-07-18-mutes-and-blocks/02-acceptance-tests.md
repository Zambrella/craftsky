...
This is a high-risk, cross-stack safety feature. The test design therefore treats the AppView as the enforcement boundary and Flutter as an account-scoped optimistic client, with the following layers:
...
- Real-Postgres integration tests verify migrations, private mute ownership, public block indexing, PDS-first mutations, immediate pre-Tap enforcement, eligible-row pagination, membership lifecycle, notifications, push cancellation, and every affected API/store path.
...
| FR-025 | AC-036, AC-037 | AT-012, UT-011, REG-009 | Acceptance / Unit / Regression | Yes |
...
Feature: Profile mute and block controls
  Scenario: An eligible visitor manages another member
    ...
    Then Follow and Share remain primary actions
    And More offers the current Mute or Unmute, Block or Unblock, and Report actions
    When Alice chooses Mute
...
| UT-010 | FR-003, FR-004, FR-006 | AC-010, AC-011, AC-012, AC-014 | Decide canonical block record and stale-event reconciliation. | Duplicate pair URIs, create/update/delete CID/version order, optimistic state generation, matching and stale Tap events. | Craftsky selects/deletes its owned active record deterministically; duplicates converge; stale events cannot reverse newer state. | `appview/internal/index/bluesky_block_test.go` |
| UT-011 | FR-025, NFR-001 | AC-036, AC-037 | Exercise Flutter optimistic mutation state machine with account key and activation generation. | Success, failure, duplicate tap, account switch, late completion, refresh callbacks. | One in-flight mutation per account/subject/action; rollback and feedback are account-local; late results cannot mutate the newly active account. | `app/test/profile/providers/profile_relationship_provider_test.dart` |
...
| IT-005 | BR-002, FR-003, RULE-002 | AC-003, AC-010, AC-046 | Block creates one canonical public PDS record before success. | Fake/recording PDS for Alice; real optimistic relationship store; current Bob. | Alice blocks Bob and immediately reads both directions. | PDS receives one `app.bsky.graph.block` with Bob DID and valid `createdAt`; response waits for PDS and immediate state; restrictions are symmetric. | `appview/internal/api/block_test.go` |
| IT-006 | FR-003, FR-006 | AC-011, AC-014 | Block/unblock retry and pre-Tap race convergence. | PDS record identity plus optimistic state; Tap delivery held. | Retry block, unblock before create event, retry unblock, then deliver create/delete out of order. | Exact owned record is deleted at most once; final canonical state wins; late events are no-ops and never resurrect restrictions. | `appview/internal/api/block_test.go`, `appview/internal/index/bluesky_block_test.go` |
| IT-007 | BR-002, FR-004, FR-028 | AC-004, AC-012, AC-051 | Index external compatible block lifecycle and replay without surfacing non-members. | Real Postgres; valid and invalid external `app.bsky.graph.block` Tap events including duplicate pair URIs/CIDs and a current-member-owned record targeting a non-member. | Deliver create/update/duplicate/stale/delete/replay sequences, then query user-facing surfaces for the absent subject. | Valid external records are retained and converge without Craftsky recreation even when the subject is absent; the non-member remains omitted and unmanageable; malformed events fail safely. | `appview/internal/index/bluesky_block_test.go` |
| IT-008 | FR-005 | AC-013, AC-059 | Membership activation is fail-closed across both block-owner directions, failure, and process restart. | Fixture A: the joining repository owns an outbound block targeting current Alice. Fixture B: current Alice already owns a retained indexed block targeting the joining DID. Real persisted activation/backfill state and a restartable service/backfiller are available. | Start activation, interrupt before and during backfill/verification, recreate the service, retry injected failures, and attempt profile reads/interactions before and after successful completion. | The account remains non-interactable through every failure and restart; unfinished work resumes or retries idempotently; Fixture A is backfilled, Fixture B is verified from retained indexed state, and both blocks enforce before visibility or interaction. | `appview/internal/index/bluesky_backfiller_test.go`, `appview/internal/auth/initialize_profile_test.go` |
| IT-009 | FR-006 | AC-014 | Immediate server policy survives optimistic-store failure and Tap lag. | PDS success; injectable synchronous-store success/failure; delayed Tap. | Block, read/interact immediately, retry after local failure, then reconcile Tap. | API never claims fully applied success if immediate enforcement failed; client can safely hide; retry converges; successful response has no lag window. | `appview/internal/api/block_test.go` |
...
| IT-025 | FR-020, FR-028, FR-029 | AC-052 | Subject leave/rejoin hides, retains, restarts, backfills, and restores relationships safely. | Alice has follow, Alice-owned block, Bob-owned block, mute, and content interactions naming Bob; Bob membership is removed then the same DID is re-added with persisted activation state. | Read/manage during absence; interrupt and restart reactivation before and during backfill/verification; retry to completion; read before and after activation. | No Bob row/count/tombstone/action appears while absent or activation is incomplete; no underlying relationship is deleted; restart cannot reopen interaction; after successful completion, retained and backfilled state reappears once before interaction is enabled. | `appview/internal/auth/initialize_profile_test.go`, relationship store/API tests |
...
| TD-003 | Canonical block event history | Valid `app.bsky.graph.block` records with explicit owner repository and subject DID: joining-owner → current-subject, current-owner → absent/joining-subject, duplicate pair URIs, create/update/delete, old create after delete, malformed subject/time, and matching optimistic generation. | UT-010, IT-005–IT-009, IT-032, IT-036, MAN-002 |
...
| TD-007 | Multi-account client state | Alice and Carol sessions, fixed account-bound repositories/clients, per-account providers/caches/lists/counts, activation generations, in-flight mutation, and revealed branch. | AT-004, AT-012, UT-011, IT-003, REG-009 |
...
| TD-011 | Membership lifecycle | Subject departure/rejoin, owner permanent removal, sign-out, device removal, account switch, persisted activation pending/backfill failed/backfill complete, process exit before/during work, service recreation, and idempotent retry. | UT-014, UT-016, IT-008, IT-024–IT-026, IT-035, REG-005 |
...
| MAN-002 | BR-002 | AC-003, AC-004 | Compatible-client/local-PDS interoperability smoke test. | Against the local full stack, create a block in Craftsky and inspect the repository; create/delete a valid `app.bsky.graph.block` with a compatible client or repo tool; allow Tap/backfill to converge; inspect both Craftsky accounts. | Craftsky writes one canonical public record, honors external create/delete without recreation, and enforces both directions before serving protected content/interactions. |
...
| GAP-005 | Membership activation and Tap backfill ordering remains safety critical at arbitrary process-failure points. | FR-005, FR-028 | IT-008 and IT-025 cover persisted boundary transitions and service recreation, but no finite test can exercise every instruction-level crash instant. | Fault-inject each persisted transition in IT-008/IT-025; require fail-closed restart behavior at every tested boundary and block implementation approval if any restart exposes the account early. |

No blocking test-design gaps are identified. GAP-001 is an explicit delivery boundary; GAP-002 and MAN-002 provide a live interoperability smoke path; GAP-003 concerns an unspecified performance threshold rather than functional coverage; GAP-005 is concretely exercised at persisted restart boundaries by IT-008 and IT-025.
...
- Suggested test order for implementation:
  1. Write `IT-035` and `IT-002` together; make `IT-035` the first red schema contract, then green only the minimum reversible migration and owner-scoped store behavior before handler-level immediate enforcement. Add `UT-001`, `UT-014`, and `UT-016` for shared policy, membership, and lifecycle semantics.
  2. `IT-005`–`IT-009`, `IT-036`, and `UT-010` for PDS blocks, Tap subscription/reconciliation, immediate enforcement, and join-time backfill.
