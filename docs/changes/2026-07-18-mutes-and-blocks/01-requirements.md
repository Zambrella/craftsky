...
- Constraints discovered:
  ...
  - Public graph writes follow the existing eventual-consistency contract: Flutter applies account-scoped optimistic state, the AppView reports success only after the PDS confirms the write, and server-side relationship enforcement converges when Tap indexes the record.
...
Decision / implication: Public follow/block records and private mute rows are not deleted merely because the subject lacks a current Craftsky profile. They are not surfaced or counted while the subject is absent. If the subject later regains membership, the `craftsky_profiles` row restores membership immediately, retained indexed state applies immediately, and joining-owned public records converge through the ordinary Tap backfill. A block against an absent subject is not manageable in Craftsky until the subject rejoins.
...
Decision / implication: Profiles keep Follow and the current Mute/Unmute action visible and move Share, Block/Unblock, and Report into one adaptive More menu. The menu renders as a bottom sheet on compact screens and an anchored popup on larger screens. Post, project-post, comment, and reply menus expose Mute/Unmute author, Block/Unblock author, and Report post. Mute acts immediately with feedback; Block and Unblock require confirmation.
...
### Q11: Must a successful block be present in AppView Postgres before the API returns?

Answer: No. Blocks should follow the existing public relationship write contract used by follows and likes.

Decision / implication: Flutter updates optimistically and rolls back on request failure. The AppView writes the canonical PDS record and returns the expected relationship state plus record identity after PDS confirmation, without synchronously inserting an optimistic block row. Server reads and policy may retain the prior state during the ordinary Tap indexing window. A rapid unblock before Tap convergence must still locate and delete the exact owned PDS record.
...
- Matches the existing PDS-confirmed write and Tap-indexed read model.
...
- Accepts a brief, observable server-read convergence window after public PDS writes.
...
Why: It satisfies the defining requirements together: a hard Craftsky membership boundary, Bluesky-compatible public blocks between members, truly private account-scoped mutes, and server-enforced restrictions across every delivery and interaction path after Tap convergence. A central relationship policy should be reused by SQL/read stores, write authorization, notification eligibility, push delivery, and response shaping, while Flutter provides immediate account-scoped optimistic UX during the accepted indexing window.
...
- G-008: Make Flutter relationship state immediate, require PDS confirmation before reporting public block success, and converge AppView enforcement through Tap consistently with other public atproto relationships.
...
| Blocking user | Signed-in Craftsky member preventing mutual viewing and interaction | Immediate account-scoped Flutter feedback, PDS-confirmed public state, Tap-converged symmetric server enforcement, and a manageable block list containing current members only. |
...
A user can mute or unmute another current Craftsky member from the visible profile action, block or unblock them from the profile More menu, use all relationship actions from post More menus, and manage current muted/blocked members from Settings. A non-member handle or DID returns the same `404 profile_not_found` boundary across account reads and mutations. Existing relationships with absent subjects remain hidden until the subject rejoins.
...
The presence of a `craftsky_profiles` row is authoritative for current membership. The AppView is authoritative for relationship enforcement from its indexed mute/block state. Flutter optimistically removes or collapses affected content and updates relationship state immediately; after Tap convergence, all reads, writes, notification eligibility, new-count computation, and pending push delivery independently enforce the same policy. During the accepted Tap window after a Craftsky public block write, Flutter retains its account-scoped optimistic overlay while server reads may still reflect the prior indexed state. Public block events from Craftsky or other compatible clients converge through the same indexer. Underlying public follow/block and interaction records are preserved; viewer-relative state, lists, search, and block-violating references are shaped without destructive rewrites.
...
| FR-003 | Functional | Must | The AppView shall create a canonical `app.bsky.graph.block` record on the caller's PDS and delete the caller-owned matching record or records when unblocking; repeated Craftsky requests shall converge without creating another active record for the pair. | The PDS record is public canonical state, while compatible clients may have created duplicate records that Craftsky must safely collapse and remove. | Prompt; block lexicon | AC-010, AC-011 |
...
| FR-005 | Functional | Must | A current `craftsky_profiles` row shall be the sole membership predicate. When an account joins or rejoins, the AppView shall request ordinary Tap repository tracking/backfill so compatible `app.bsky.graph.block` records owned by that repository converge into the block index; already-indexed blocks owned by current members and targeting the joining DID shall apply as soon as the profile row exists. | Membership should use the existing profile boundary without a second activation lifecycle, while public relationship state retains ordinary Tap eventual consistency. | Grilling decision; interoperability goal; User decision | AC-013, AC-059 |
| FR-006 | Functional | Must | Flutter shall optimistically reflect block/unblock immediately and roll back on request failure; the AppView shall report success only after the canonical PDS mutation succeeds, return the expected relationship state and block record identity, and rely on Tap for server-side projection and policy convergence rather than synchronously writing an optimistic block row. | This matches the existing follow/like PDS-write and Tap-indexed read contract and avoids a second relationship state machine. | Existing follow/like architecture; User decision | AC-014 |
...
| FR-022 | Functional | Must | On an eligible unblocked visitor profile, Flutter shall keep Follow and the localized current Mute/Unmute action visible and place Share, Block/Unblock, and Report in an adaptive More menu that renders as a bottom sheet on compact screens and an anchored popup on larger screens; Block and Unblock shall each require confirmation, and Mute/Unmute shall apply immediately with feedback. Blocked-profile restrictions in FR-016 take precedence. | The revised action hierarchy prioritizes the safety preference while preserving access to sharing and making public block changes deliberate. | User polish decision | AC-033 |
...
| FR-025 | Functional | Must | Flutter shall optimistically update the active account's relationship state and currently loaded content when a mutation starts, roll back on failure, merge the PDS-confirmed response on success, retain the successful block/unblock overlay across stale server refreshes until Tap convergence, refresh affected feeds/profile/notification counts, and never apply the state to another signed-in account. | Immediate UX and eventual server reads must not cause flicker, rollback after success, or multi-account leakage. | Reference architecture; multi-account client; User decision | AC-036, AC-037 |
...
| AC-002 | BR-001 | Given a relationship is present in the AppView's indexed state, when any equivalent Craftsky surface is read or acted upon, then the server and client apply the same relationship policy. |
...
| AC-010 | FR-003 | Given no block exists, when block succeeds, then the PDS write succeeds before the API reports success and the response identifies the created URI, CID, and rkey without requiring a synchronous `atproto_blocks` insert. |
| AC-011 | FR-003 | Given one or more caller-owned records block the same subject, when unblock or either operation is retried, then every matching owned rkey is deleted idempotently and Craftsky does not create another active record while any match exists. |
| AC-012 | FR-004 | Given create, update, duplicate replay, and delete Tap events in repository order, when indexed, then URI/CID and active-pair state converge idempotently without duplicate active relationships. |
| AC-013 | FR-005 | Given a joining account owns historical `app.bsky.graph.block` records targeting current members, when its Craftsky profile is indexed and repository backfill proceeds, then the profile row immediately establishes membership and each historical block becomes server-enforced as Tap indexes it; no separate activation row or readiness state is required. |
| AC-014 | FR-006 | Given Flutter optimistically applies a block or unblock and the PDS mutation succeeds while Tap delivery is held, when the API responds then it returns the expected relationship state and record identity without a local block projection; Flutter retains that state while immediate server reads may show the prior indexed state, and server policy converges after the matching Tap event. |
...
| AC-033 | FR-022 | Given an eligible unblocked visitor profile, when it renders, then Follow and the current localized Mute/Unmute icon are visible and Share is not a standalone action; when More opens, it uses a bottom sheet on compact screens or an anchored popup on larger screens and offers Share, current Block/Unblock, and Report; Mute/Unmute applies with immediate feedback, both Block and Unblock open confirmations describing their consequences, and a blocked profile instead follows FR-016. |
...
| AC-036 | FR-025 | Given a mutation is in flight, when it succeeds or fails, then Flutter optimistically updates once, blocks duplicate taps, rolls back on failure, shows feedback, and refreshes affected server-backed state; after success, a stale pre-Tap response cannot reverse the confirmed optimistic relationship before indexed state converges. |
...
| AC-052 | FR-020, FR-028, FR-029 | Given Bob leaves Craftsky while Alice has an existing public follow/block record or private mute row naming Bob, when Bob is absent then all such relationships are hidden and unmanageable without deleting them; when the same DID's profile row returns, retained indexed state reappears immediately and joining-owned public state converges through ordinary Tap backfill. |
...
| AC-059 | FR-005 | Given an already-current member owns an indexed historical block targeting a non-member, when that target later gains a `craftsky_profiles` row, then the retained indexed block applies immediately because both DIDs are current members; no activation verification state is required. |
...
| EC-004 | PDS block succeeds while Tap delivery is delayed | API returns the PDS-confirmed record identity and expected relationship state; Flutter retains its optimistic overlay while AppView server reads converge through Tap. | FR-003, FR-006 |
...
| EC-006 | Tap replays duplicate create/delete events | URI/CID-keyed indexing is idempotent and the repository-ordered final event determines indexed state. | FR-004, FR-006 |
...
| EC-015 | User unblocks before Tap indexes the create | If the active row is not yet indexed, the AppView locates the caller-owned record on the PDS by subject and deletes its exact rkey; Flutter remains unblocked while the ordered create/delete Tap events converge. | FR-003, FR-006 |
...
| EC-022 | Blocked or muted subject leaves then rejoins | No list tombstone or management row is shown while absent; retained indexed state reappears when the profile row returns, while joining-owned public blocks converge through Tap. | FR-005, FR-020, FR-029 |
...
| EC-024 | Process exits during membership block backfill | Membership remains defined solely by `craftsky_profiles`; a later OAuth/repository-tracking request or Tap retry resumes ordinary indexing, and any temporary relationship-policy lag remains within the accepted convergence contract. | FR-005 |
...
- New public indexed data:
  - Active `app.bsky.graph.block` rows retaining URI, blocker DID, rkey, CID, subject DID, record JSON, created time, and indexed time. URI and owner/rkey are unique for idempotent projection; blocker/subject is indexed but may contain compatible-client duplicates that policy/list reads collapse.
...
- Tap/indexing impact:
  ...
  - Keep membership defined solely by the presence of a `craftsky_profiles` row; do not add a second activation/readiness table.
  - Request ordinary Tap repository tracking/backfill during join/rejoin. Retained indexed blocks targeting the joining DID apply immediately once membership exists; joining-owned historical blocks become effective as Tap indexes them.
- Existing data:
...
- Backwards compatibility:
  ...
  - Older clients may not render optimistic controls; server enforcement is mandatory after Tap convergence.
  - Existing public block records are compatible by design.

## 16. UI / API / CLI Impact

- UI:
  - Visitor profiles keep Follow and the current Mute/Unmute action visible; their adaptive More menu contains Share, Block/Unblock, and Report.
...
- API (exact handler naming may be refined during planning while preserving these contracts):
  ...
  - `POST /v1/profiles/{handleOrDid}/blocks` — create the public block on the PDS; return updated relationship plus URI/CID/rkey without synchronously inserting an indexed block row.
...
| RISK-003 | PDS write and AppView/Tap state temporarily diverge. | Another client or raw API may observe the prior policy during the indexing window, or a stale refresh may try to overwrite Flutter's optimistic state. | Make the accepted window explicit; retain the account-scoped Flutter overlay until convergence; monitor Tap lag/failures; use idempotent indexing and exact PDS lookup for rapid unblock. |
...
| RISK-012 | A joining profile becomes a member before all joining-owned historical blocks are indexed. | The account may briefly view or interact under the prior indexed relationship state. | This is an accepted consequence of using `craftsky_profiles` as the sole membership predicate; request Tap backfill during onboarding, retain already-indexed inbound blocks, observe lag/failures, and test eventual convergence after restart/retry. |
...
| ASM-009 | A current `craftsky_profiles` row is the complete canonical server-side membership signal; no separate activation/readiness state exists. | Membership predicates and lifecycle tests would need revision if block-backfill completion later becomes part of membership eligibility. |
...
Notes: Document review is recorded in `03-document-review.md`. The 2026-07-19 revision accepts ordinary Tap convergence for public block writes and uses `craftsky_profiles` as the sole membership predicate. Approval permits coding-plan work only; implementation still requires explicit approval.
...
- Suggested test levels:
  ...
  - Database: migrations, mute-pair and block URI/rkey uniqueness, block pair-direction indexes, owner deletion versus subject retention, eligible-member pagination, notification/push cancellation.
  - Indexer: create/update/delete/duplicate replay in repository order, duplicate pairs, retention of current-member-owned blocks targeting absent subjects, joining-owned historical backfill, restart/retry convergence, and notification interaction.
  - Handler/integration: every read/write route in the affected inventory, uniform non-member 404/omission, PDS success/failure, PDS-confirmed response plus delayed-Tap behavior, rapid pre-index unblock, and standard errors.
  ...
  - End-to-end: two-account mute, one-way/inbound/mutual block, third-party block reference, leave/rejoin, both join-time record-owner directions, Tap interruption/process restart, owner deletion, rapid block/unblock before indexing, deep links, notification restoration, push eligibility, and existing-record restoration.
