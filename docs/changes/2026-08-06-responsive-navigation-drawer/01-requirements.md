...
Decision / implication: Move `SavedPostsRoute`, `ScheduledPostsRoute`, and `DraftsRoute` from children of `SettingsRoute` to sibling children of `ProfileRoute`, while presenting them through the authenticated shell navigator. Their paths become `/profile/saved`, `/profile/scheduled`, and `/profile/drafts`. `SavedPostFolderRoute` remains a child of `SavedPostsRoute`, becoming `/profile/saved/folder`. Remove the three corresponding rows from `SettingsPage`. Because the app has no production users, do not add legacy `/profile/settings/...` redirects or aliases.

### Q9: Should detail pages and modal composers cover responsive navigation?

Answer: On compact screens, yes: detail pages and composers retain their existing full-screen presentation without the bottom navigation. On large screens, no: the navigation rail remains visible beside signed-in detail, management, and modal-composer content.

Decision / implication: Add an authenticated shell outside the existing stateful branch shell. It renders no additional chrome on compact screens, but supplies the persistent rail for large-screen routes lifted above the branch navigators. Responsive modal navigation uses the root navigator on compact screens and the nearest authenticated content navigator on large screens.

### Q10: Which navigation operations should replace the current location versus add a transient page?

Answer: Application-section and management navigation should normally use declarative `go` semantics. A true drill-in that must return to its exact origin, such as opening post details or a compact profile card, may continue to use `push`; modal flows continue to use their responsive navigator.

Decision / implication: Drawer, rail, Settings rows, personal-content folders, scheduled-content management, notification outcomes, and notification-settings actions replace the current route configuration. Post/thread drill-ins and profile-card drill-ins remain imperative pushes so Back returns to their originating feed, Saved list, Search result, or notification list. The authenticated shell owns the large rail independently of whether its child route was reached by `go` or `push`.
...
At laptop and desktop widths, the shell presents the same core, secondary, legal, and feedback destinations in an extended rail/menu layout, with footer actions visually separated from route destinations. The rail remains visible while navigating through signed-in detail routes, other members' profiles, personal-content and management pages, and modal composers. No drawer or compact bottom bar is shown. Both layouts retain active-route feedback, notification newness, the Profile account affordance, responsive behavior, localization, and accessibility.

At mobile and tablet widths, pushed detail routes and modal composers retain their existing full-screen presentation and cover the compact bottom navigation. They continue to use Back or Close rather than gaining a drawer hamburger.
...
| FR-002 | Functional | Must | At large form factors, the shell shall show the extended navigation rail/menu throughout signed-in branch, detail, personal-content, management, and modal-composer routes, and shall not show the compact drawer or bottom navigation bar. | Preserves the established responsive split while making large-screen navigation persistent. | Prompt; User follow-up; Codebase | AC-002, AC-020, AC-021 |
...
| FR-005 | Functional | Must | Core destinations shall retain existing stateful-shell behavior. Secondary destinations shall reuse the existing page implementations through the authenticated shell navigator, using their updated typed routes. On a small screen, selecting any drawer item shall first close the drawer and then perform at most one navigation action. | Prevents duplicate pages, stale overlays, and regression in branch or Back behavior while allowing responsive presentation and the required hierarchy change. | Codebase; Recommended direction; User follow-up | AC-005, AC-013, AC-018, AC-020 |
...
| FR-010 | Functional | Must | The hamburger action shall appear only at the five branch roots on small screens. Detail, editor, management, Settings, and modal pages shall retain their appropriate Back or Close affordance. | Prevents the drawer action from obscuring navigation hierarchy or replacing expected Back behavior. | Prompt; Codebase; Q3 | AC-010 |
...
| FR-015 | Functional | Must | On compact form factors, detail, personal-content, management, and modal-composer routes shall remain full-screen, cover the bottom navigation, and retain their appropriate Back or Close affordance. | Preserves the explicitly confirmed compact interaction model. | User follow-up | AC-020 |
| FR-016 | Functional | Must | Full-screen modal flows launched from authenticated content shall use responsive navigator selection: the root navigator on compact form factors and the nearest authenticated content navigator on large form factors. | Keeps compact modals immersive while preventing them from covering the persistent large-screen rail. | User follow-up; Codebase | AC-021 |
| FR-017 | Functional | Must | Navigation between application sections and hierarchy-owned management routes shall use declarative replacement semantics. Imperative pushes shall be reserved for transient drill-ins that must return to their exact origin and for modal/result flows. The authenticated shell shall retain the large rail for either navigation operation. | Prevents stacked duplicate shells and stale full-screen overlays while preserving useful detail-page Back behavior. | User follow-up; go_router behavior | AC-020, AC-022 |
...
| AC-002 | FR-002, RULE-001 | Given the app width is above 900 logical pixels, when any signed-in route within the authenticated application shell renders, then the extended rail is visible and neither a drawer hamburger nor compact bottom bar is shown. |
...
| AC-005 | BR-002, FR-005 | Given the member selects a core destination, then the existing stateful-shell branch behavior runs exactly once; and given they select Saved posts, Scheduled posts, Drafts, or Settings, then the corresponding typed route opens exactly once with its responsive shell presentation and Back behavior and no duplicate page is introduced. On a small screen, the drawer is no longer visible after the selection. |
...
| AC-010 | BR-001, FR-010, RULE-002 | Given the member is on a detail, management, Settings, editor, or modal page, when its app bar renders, then its existing Back or Close affordance remains authoritative and this change does not replace it with a hamburger; returning to a branch root restores small-screen drawer access. |
...
| AC-020 | FR-002, FR-005, FR-015 | Given a signed-in detail, other-member profile, personal-content, or management route renders, then at widths above 900 logical pixels the rail remains visible beside the route and reflects the corresponding menu selection; at widths at or below 900 logical pixels the route remains full-screen with no rail or bottom navigation. |
| AC-021 | FR-002, FR-016 | Given the member opens a post or project composer, then at widths above 900 logical pixels the modal covers only the authenticated content area and the rail remains visible; at widths at or below 900 logical pixels the modal uses the root presentation and covers the compact bottom navigation. |
| AC-022 | FR-005, FR-017 | Given the member selects Saved, Scheduled, Drafts, Settings, a Settings child, a saved folder, or a notification outcome, then navigation replaces the current declarative destination rather than stacking another application shell. Given a post/thread or compact-profile drill-in is pushed from authenticated content, then Back returns to its exact origin and the large rail remains visible throughout at large widths. |
...
| EC-005 | A detail or secondary route is opened at either form factor. | On compact screens it covers the bottom navigation and retains full-screen Back behavior; on large screens it remains beside the rail and preserves its Back behavior. | FR-002, FR-005, FR-010, FR-015 |
...
- UI:
  ...
  - Keep the rail visible beside signed-in detail, management, personal-content, and modal-composer routes on large screens.
  - Keep detail and modal-composer routes full-screen over the bottom navigation on small screens.
...
- Router:
  - Add an authenticated shell navigator outside the existing stateful branch shell so responsive navigation chrome can persist independently of branch content.
...
| RISK-004 | Authenticated-shell secondary and detail routes conflict with shell selection or Back behavior. | Duplicate screens, lost branch state, hidden rail, or confusing selected state. | Reuse existing typed routes and navigation semantics; cover compact and large route entry, selection, and return paths in integration tests. |
...
| RISK-009 | A full-screen modal is pushed onto the wrong navigator after the persistent rail is introduced. | A large-screen composer covers the rail, or a compact composer exposes the bottom navigation. | Centralize responsive modal navigator selection and test both form factors with nested navigators. |
...
Reviewer:
Date: 2026-08-06
Notes: Review is recommended because the change crosses the responsive shell, five independently owned top-level app bars, stateful routing, a typed route hierarchy relocation, responsive nested navigators, Settings content, account/notification navigation state, native drawer gestures, and accessibility. The user explicitly approved direct TDD implementation without the intermediate workflow stages.

## 23. Handoff To Test Design

- Requirements file: `01-requirements.md`
- Next test specification: `02-acceptance-tests.md`
- Must-cover requirement IDs: BR-001–BR-004, FR-001–FR-017, NFR-001–NFR-004, RULE-001–RULE-005
- Suggested test levels:
  ...
  - Flutter router/integration tests for core branch selection/reselection, relocated secondary typed routes, positive new paths, negative former paths, responsive shell Back behavior, persistent large-screen rail, compact full-screen details, responsive composer navigator selection, resize across the breakpoint, external legal links, and Feedback action/failure.
