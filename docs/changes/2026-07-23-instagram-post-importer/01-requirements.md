...
- Constraints discovered from the consented local July 2026 Instagram export:
  ...
  - In a current dual-representation export, a legacy post can leave its root
    caption empty while retaining the caption on its media item, and the
    detailed representation can contain several `Creation time` values that
    do not identify the post itself. For equivalence and publication dates,
    use the first non-empty supported caption and prefer media-attached
    timestamps before labelled or root fallbacks.
  - Current Posts metadata can reference media in the flat `media/posts`
    directory, a six-digit date-partitioned `media/posts` subdirectory, or the
    flat `media/other` directory. These exact metadata-linked forms are post
    media; arbitrary nesting and unrelated media directories remain excluded.
...
result in review without a warning, and leave ambiguous text unchanged.
...
offer explicit CID-preconditioned rollback for those records. Rollback does not
target unrelated records. AT record CIDs identify content rather than record
incarnations, so a byte-identical record deleted and later recreated at the
same rkey cannot be distinguished from the originally imported record.
...
event. The label must not imply Instagram account ownership. CraftSky-aware
edit paths must retain the provenance field. An arbitrary third-party client
can replace the public record without that optional field; AppView then reflects
the replacement record rather than inventing sticky provenance.
...
substituting the current time. Merge equivalent duplicate representations,
using media-attached timestamps before labelled or root fallbacks and the
first non-empty caption available within each supported representation. Skip
a materially conflicting duplicate rather than guessing.
...
subdomain with separate metadata. A development URL using the `localhost`
hostname is canonicalized to `127.0.0.1` before archive or OAuth state is
created because AT Protocol loopback callbacks require an IP literal.
...
| FR-007 | Functional | Must | The parser shall recognize only explicitly supported, versioned Instagram post metadata paths and shapes, including an optional single top-level export directory. Metadata-linked media paths shall be restricted to the observed flat `media/posts`, six-digit date-partitioned `media/posts`, and flat `media/other` forms; arbitrary nesting and unrelated media directories shall remain excluded. The parser shall fail locally with guidance for absent, ambiguous, encrypted, malformed, or unsupported inputs. | Instagram's export shape is undocumented and untrusted, while current exports use more than one Posts-owned media directory form. | Discovery / Local sample observation | AC-005, AC-019 |
| FR-008 | Functional | Must | The parser shall merge equivalent supported `posts*.json` representations and deterministically deduplicate them using normalized timestamps and media identities, without treating caption equality alone as identity. Within each supported representation it shall select the first non-empty supported caption and prefer media-attached timestamps before labelled or root fallbacks. Materially conflicting representations shall produce one skipped ambiguity item rather than an automatic preference. | Current exports can repeat one post across shapes while placing the authoritative caption and timestamp at different nesting levels; genuine conflict must not be guessed. | Local sample observation / Requirements grilling | AC-005, AC-019 |
...
| FR-010 | Functional | Must | The review UI shall virtualize and filter every importable or skipped post; select all importable posts by default, including warned transformations; expose bulk and per-post selection, original date, final editable caption, selected supported media, warning reasons, and aggregate selected/skipped/transformed counts; and require a final destination-account confirmation with exact post/image counts before publication. Each mounted post row shall automatically load its first available sanitized image through the existing one-at-a-time worker queue without generating a separate thumbnail asset. Each post's caption and image sections shall remain independently collapsible and initially collapsed, the image heading shall state the image count, and an expanded caption editor shall grow to show its full content. Expanding the image section shall progressively load the remaining selected images through that queue, show image previews at least 132 CSS pixels square, and open a selected preview in an accessible full-screen lightbox that closes by its button, Escape, or backdrop. | Makes very large bulk publication practical and visually recognizable while virtualization and worker serialization bound simultaneous row and image work. | User answers / Requirements grilling / UI follow-up | AC-006 |
...
| FR-023 | Functional | Must | CraftSky post responses and Flutter presentation surfaces, including quoted-post previews, shall render a subtle localized "Imported from Instagram" label for imported posts and no label for ordinary posts. Every CraftSky-aware edit path shall preserve the public provenance field; replacement by a third-party client that omits the optional field removes the classification. | Provides durable origin context without claiming that arbitrary public-record replacements can be controlled or implying current text is unchanged. | User answer / Requirements grilling / Document review | AC-011 |
...
| FR-025 | Functional | Must | The importer shall persist bounded progress keyed to the authenticated DID and a locally derived archive-manifest fingerprint, including successful AT URIs, the public CIDs returned for records created and durably claimed by that import session, ownership state, and safe status/error codes, but not raw archive bytes, captions, image bytes, OAuth tokens, or unrelated source metadata; this history shall survive sign-out and remain until explicit clearing or successful rollback. OAuth artifacts may exist only in storage owned by the standards-based OAuth library, shall never be copied into importer progress or diagnostics, and shall be removed through the library's sign-out flow. | Supports resume and CID-guarded rollback without retaining sensitive source content while allowing the OAuth client to manage its required session state. | User answer / Privacy boundary / Requirements grilling / Document review / Coding-plan audit | AC-014, AC-017 |
...
| FR-027 | Functional | Must | Cancel shall stop scheduling new uploads without deleting successful posts; explicit rollback shall target only locally tracked post records whose create result and returned CID were durably claimed by the selected import session, shall use that CID as the delete precondition, and shall report per-record success, absence, or conflict. An edited or replaced record, a recreated record whose current CID differs, and every pre-existing or ownership-ambiguous record shall not be deleted. A byte-identical record deleted and recreated at the same rkey has the same content CID and cannot be distinguished by this mechanism; the rollback confirmation shall disclose that residual limitation. | Separates pause from destructive recovery, prevents rollback from deleting content-changed public-record replacements, and states the AT record-CID boundary truthfully. | User answer / Coding-plan audit / Implementation review | AC-015 |
...
| AC-006 | BR-004, FR-010 | Given a parsed manifest containing thousands of valid, transformed, skipped, and unsupported posts, when review opens, then all importable items are selected by default, the virtualized list remains filterable without requiring every item to open, bulk/per-item selection and each applicable detail are available, caption and image sections start collapsed and expand independently, image headings state their image count, an expanded caption editor shows its full content height, and an expanded image section progressively loads selected thumbnails at least 132 CSS pixels square through the one-at-a-time worker queue. Selecting a thumbnail opens an accessible full-screen lightbox that closes by its button, Escape, or backdrop. Aggregate counts remain exact, and the first network write is blocked until a final destination-account confirmation states exact selected post and image counts. |
...
| AC-011 | BR-001, FR-018, FR-019, FR-020, FR-023, RULE-002 | Given a valid selected historical post, when imported, indexed, rendered directly or as a quoted-post preview, and later replaced through a CraftSky-aware edit fixture that retains the field, then its PDS record has original `createdAt`, deterministic `tid`, minimal self-asserted Instagram provenance and no source identifiers; AppView returns the provenance, profile chronology uses the original date, and Flutter retains a subtle localized label without implying verified ownership or unchanged content. Given a third-party replacement that omits the optional field, AppView and Flutter no longer classify the record as imported. |
...
| AC-014 | BR-004, FR-024, FR-025, FR-026, FR-028, RULE-007 | Given success, interruption before and after PDS create, reload, sign-out/sign-in, transient failure, already-imported, edited-existing, deleted-previous, deterministic-rkey-collision, and a later overlapping export, when manifests are resumed or started, then safe progress survives sign-out until explicit clearing/successful rollback, successful records are not duplicated or overwritten, remaining/failed/new records can continue, a deleted record can be recreated, only a durably stored create result/CID establishes rollback ownership, a create with an ambiguous or lost local result resumes as existing-unowned, a newer session never claims earlier rollback targets, collisions fail safely, and clearing explains loss of resume/bulk rollback. |
| AC-015 | BR-004, FR-027, FR-028, RULE-006 | Given a partial import with owned successes, pre-existing/unowned records, failed items, an owned record edited or replaced after import, and unrelated posts, when the member cancels, no new work starts and existing posts remain; when they explicitly roll back, only CID-matching records durably owned by that session are targeted, absent/conflicting/per-record failures are reported, and edited, replaced, CID-different recreated, pre-existing, ownership-ambiguous, unrelated records and blobs are never deleted. Before confirmation, the UI discloses that a byte-identical delete-and-recreate at the same rkey cannot be distinguished from the original import by content CID. |
...
| AC-017 | BR-002, FR-009, FR-019, FR-025, NFR-001, NFR-005, RULE-008 | Given synthetic canaries in every raw archive category, importer-progress field, returned record CID, and OAuth adapter boundary, when parsing, review, failure, resume, publication, diagnostics, sign-out, and rollback execute, then raw/source canaries are absent from PDS records except explicitly selected final public content, and absent from importer-owned persistence, logs, telemetry, URLs, snapshots, errors, and stringification. Importer storage contains only the approved content-free progress fields including a created record's public CID, while that CID is absent from diagnostic output. OAuth material is confined to the OAuth library's private storage, never copied into importer state or diagnostics, and removed by its sign-out flow. |
...
- Flutter:
  - Extend post wire/model state with minimal provenance for presentation and
    include it in both full post and quoted-post response shapes.
  - Preserve it through every CraftSky-aware edit path. Do not make AppView
    provenance sticky when the authoritative public PDS record removes it.
- Browser:
  - OAuth library-managed session state in its own IndexedDB storage, removed
    through the library's sign-out flow and never copied into importer state.
  - Separate bounded importer progress containing DID binding, manifest
    ...
    created AT URIs plus returned public record CIDs for CID-guarded rollback.
...
- Web UI:
  ...
  - Clear warnings for omitted images/video, skipped posts, caption
    truncation, unsupported variants, and public publication. Confidently
    repaired caption text is shown without a warning.
...
| RISK-012 | A tracked record is deleted and later recreated byte-identically at the same rkey | CID-guarded rollback cannot distinguish the new incarnation and may delete it | Target only durably owned records, use `swapRecord`, disclose the content-CID limitation at rollback confirmation, and direct users to clear local history when they no longer want rollback authority |
