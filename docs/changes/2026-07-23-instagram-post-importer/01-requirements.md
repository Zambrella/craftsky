...
| FR-010 | Functional | Must | The review UI shall virtualize and filter every importable or skipped post; select all importable posts by default, including warned transformations; expose bulk and per-post selection, original date, final editable caption, selected supported media, warning reasons, and aggregate selected/skipped/transformed counts; and require a final destination-account confirmation with exact post/image counts before publication. Each post's caption and image sections shall be independently collapsible and initially collapsed, the image heading shall state the image count, and an expanded caption editor shall grow to show its full content. Expanding the image section shall progressively load every selected image through the existing one-at-a-time worker queue, show thumbnails at least 132 CSS pixels square, and open a selected thumbnail in an accessible full-screen lightbox that closes by its button, Escape, or backdrop. | Makes very large bulk publication practical, understandable, and deliberate without increasing peak image-decode memory. | User answers / Requirements grilling / UI follow-up | AC-006 |
