...
- G-002: Prevent incomplete or invalid image posts from being submitted.
...
- NG-003: Client-side image compression, resizing, transcoding, EXIF stripping, or other byte transformation.
...
- NG-007: Manual image reordering UI.
...
| FR-001 | Functional | Must | The system shall let a signed-in user select up to 4 images while composing a new top-level post. | Matches the backend/media contract and confirmed scope. | Discovery, backend workflow | AC-001, AC-007 |
| FR-002 | Functional | Must | The system shall validate selected images client-side where practical against the supported image contract: JPEG, PNG, or WebP, and a maximum size of 15 MB per image. | Reduces avoidable backend failures and follows the request to validate as much as possible in the app. | User prompt, discovery | AC-007 |
...
| FR-004 | Functional | Must | The system shall upload original selected image bytes unchanged and shall not resize, compress, transcode, or otherwise transform image content in this pass. | Preserves the confirmed validate-only/originals decision. | User answer, discovery | AC-009 |
...
| FR-011 | Functional | Must | The top-level post submit action shall remain disabled unless the post text is valid, no selected image is still uploading, no selected image is in unresolved failure state, and all selected images have non-empty alt text. | Enforces a complete, valid draft before create-post. | User prompt, discovery | AC-014 |
...
| RULE-002 | Business rule | Must | A draft may contain at most 4 selected images. | Matches the backend/media contract. | Discovery, backend workflow | AC-007 |
...
| AC-007 | FR-001, FR-002, FR-012, RULE-002 | Given a user selects supported images within the 4-image cap, when the app validates them, then valid images are accepted into the draft; and given an unsupported type, oversize file, or a selection that would exceed 4 images, when validated, then the invalid image or excess addition is rejected with user-visible feedback and is not uploaded. |
...
| AC-009 | FR-004 | Given an image selected for upload, when the app uploads it, then the upload uses the original selected bytes without client-side resizing, compression, transcoding, or other content transformation. |
...
| AC-012 | FR-008, NFR-002, RULE-003 | Given a draft with one or more selected images, when any image is missing alt text, then submission remains disabled; and when image content is rendered or exposed as a gallery entry point, then its alt text is available through accessibility semantics. |
...
| AC-014 | FR-011, NFR-004, RULE-003 | Given a top-level post draft, when the text is empty or too long, an image is still uploading, an image is in unresolved failure, or an image is missing alt text, then the submit action is disabled; and when all those conditions are satisfied, then the submit action becomes enabled. |
...
|---|---|---|---|
| EC-001 | User selects more than 4 images across multiple add actions | Only the first 4 accepted draft images remain allowed; excess images are rejected with feedback and are not uploaded. | FR-001, FR-012, RULE-002 |
...
- UI:
  - Top-level composer gains image selection, local preview, upload progress/error state, alt text entry, deletion, and submit gating.
  - Feed cards gain inline image carousel rendering.
  - Full-screen image gallery is added.
- API:
