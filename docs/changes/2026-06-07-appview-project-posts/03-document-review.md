...
- Missing or weak coverage: No missing Must coverage found. Weak/implementation-sensitive areas are already identified as GAP-002 for unknown open-union details and GAP-003 for migration/backfill harness availability.
...
- Risk level: High, matching both source documents. The risk comes from schema migration/backfill, Tap indexing convergence, PDS write payload validation, public `/v1/*` response shape changes, and response hydration across multiple existing post-shaped surfaces.
- Review requirement: Satisfied for document review. No blocking issues were found, but the coding plan should keep explicit risk checkpoints for migration/backfill, unknown details, indexes, and hydration coverage.
...
- Recommended first step: Start with `IT-001` — a migration/schema/backfill/index test proving minimal base project flags, `craftsky_project_posts`, FK/cascade behavior, backfill from `record JSONB`, and supporting indexes.
