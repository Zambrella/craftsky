...
- Current behavior:
  ...
  - Clarification from 2026-06-09 grill-me review: profile Posts and Projects tabs are distinct. Profile `postCount` and profile post lists exclude project posts; timeline/feed surfaces still include project posts.
...
When a standalone `social.craftsky.feed.post` contains a valid `project` object, the AppView indexes it as both a post record and a project post. Standalone means the record has no reply pointer and no quote embed. The base post row records that it is a project and carries cheap project filtering data. A dedicated project table stores the project payload and materialized query dimensions. Post create accepts the lexicon-shaped project object only for standalone posts, writes it to the user's PDS, and returns a synthetic project-bearing post response. Post-shaped timeline/feed read paths include project metadata for project posts. Profile reads return separate post and project counts, profile post lists exclude project posts, and a profile project list endpoint returns only standalone project posts.
...
| FR-008 | Functional | Must | `POST /v1/posts` shall reject malformed project request bodies, unsupported create-time craft types, `project` combined with `reply`, `project` combined with a quote embed, and client-supplied `createdAt` with standard AppView validation or bad-request errors. | Maintains current request validation posture, server-stamped timestamps, and the product rule that projects are standalone profile artifacts. | Existing post_request pattern, 2026-06-09 clarification | AC-006, EC-004 |
...
| FR-010 | Functional | Must | Existing post-shaped read/list endpoints shall hydrate project metadata consistently where project posts are eligible for that surface: single post reads, timeline/feed, notifications, project list, and create responses. Profile post lists shall exclude project posts under the split-tab product rule. | Prevents missing project data on surfaces that can return projects while keeping profile Posts separate from Projects. | Existing response reuse, 2026-06-09 clarification | AC-007, AC-008 |
| FR-011 | Functional | Must | Profile reads shall calculate `projectCount` from indexed visible standalone project posts and calculate `postCount`/recent post counts from visible non-project root posts. | Existing profile response exposes projectCount, and product wants separate Posts and Projects tabs/counts. | Codebase finding, 2026-06-09 clarification | AC-009 |
| FR-012 | Functional | Must | The AppView shall provide an authenticated, paginated profile project list endpoint that returns only standalone project posts for a resolved profile DID, ordered by the existing profile post ordering key. | Enables clients to list a profile's projects without mixing comments, quotes, or non-project posts. | Prompt, profile Projects tab finding, 2026-06-09 clarification | AC-010 |
| RULE-001 | Business rule | Must | A project post is a standalone `social.craftsky.feed.post` whose record contains `project.common.craftType`; absence of `project`, missing/empty `craftType`, any reply pointer, or any quote embed means the record behaves as a general post in AppView. | Mirrors lexicon semantics while applying the product rule that projects are standalone artifacts. | Lexicon findings, 2026-06-09 clarification | AC-001, AC-002 |
| RULE-002 | Business rule | Must | Project posts shall remain eligible for existing post behaviors including likes, reposts, moderation filtering, reporting, timeline inclusion, and deletion because they are still post records; project creation shall not allow replies or quotes. | Preserves one-record-type architecture while keeping project/profile semantics unambiguous. | ADR 001, existing routes, 2026-06-09 clarification | AC-007, AC-012 |
| RULE-003 | Business rule | Must | Project-only profile lists and project counts shall include standalone project posts only. Profile post lists and `postCount` shall exclude project posts. Timeline/feed surfaces shall still include project posts. | Keeps profile tab counts/lists aligned while preserving mixed chronological feed behavior. | Existing profile count predicates, 2026-06-09 clarification | AC-009, AC-010 |
...
| AC-002 | FR-001, FR-004, RULE-001 | Given an indexed create event for a general post, a post with `project` but no non-empty `craftType`, a project reply, or a project quote, when the indexer handles it, then `craftsky_posts` records it as non-project and no `craftsky_project_posts` row exists; given a standalone create event with `project.common.craftType`, then both the base post row and project materialization row exist. |
...
| AC-006 | BR-002, FR-007, FR-008, NFR-004 | Given malformed project JSON, missing required `project.common.craftType`, unsupported create-time craft type, wrong field types, `project` with `reply`, `project` with quote, or disallowed `createdAt`, when creating a post, then the AppView returns the appropriate standard error response and does not write a PDS record. |
| AC-007 | FR-009, FR-010, RULE-002, NFR-001, NFR-004 | Given indexed project posts are returned by project-eligible post-shaped paths such as single-post reads, timeline/feed, profile projects, notifications, or create responses, when a client reads those endpoints, then each project post response includes `project` and no endpoint performs PDS read-through to hydrate it. |
...
| AC-009 | FR-011, RULE-003 | Given a profile has a mix of root general posts, standalone project posts, project replies/quotes, and moderated posts, when the profile is read, then `projectCount` equals only visible standalone project posts and `postCount`/recent post counts equal visible non-project root posts. |
| AC-010 | FR-002, FR-003, FR-012, RULE-003, NFR-001, NFR-004 | Given a profile has indexed project and non-project posts, when `GET /v1/profiles/{handleOrDid}/projects` is requested with pagination, then the response contains only standalone project `PostResponse` items with project metadata and an opaque cursor when more rows remain. |
...
| EC-005 | A record contains `project` and is a reply/comment or quote. | The AppView create endpoint rejects this shape. If observed from the firehose anyway, the AppView preserves the raw post record but treats it as a non-project post: no project materialization, no project response field, no profile project count/list inclusion. | RULE-001, RULE-002, RULE-003 |
...
- Changed fields:
  ...
  - `profile.projectCount` changes from hardcoded zero to data-driven standalone project-post count.
  - `profile.postCount` and recent post counts exclude project posts so profile Posts and Projects tabs align.
- Migration required:
...
| ASM-003 | Profile project lists should include standalone project posts only, while profile post lists/counts exclude projects. | If replies, quotes, or profile Posts tab surfaces should include project metadata, profile counts/lists and tests would change. |
...
- [x] Resolved 2026-06-09: Profile `postCount` and profile post lists exclude project posts. The main timeline/feed still includes project posts.
