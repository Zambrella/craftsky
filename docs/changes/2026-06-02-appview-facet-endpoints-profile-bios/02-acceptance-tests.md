...
| NFR-002 | AC-005, AC-007, AC-014, EC-007 | UT-001, UT-003, UT-005, IT-001, IT-004, GAP-001 | Unit / Integration | Yes |
| NFR-003 | AC-010, AC-020, EC-003, EC-004 | AT-005, UT-009, UT-010, IT-008, GAP-003 | Acceptance / Unit / Integration | Yes |
...
| UT-004 | FR-003, FR-004, RULE-001 | AC-006; EC-001 | Map exact mention resolution outcomes to success, omitted facet target, or `mention_not_found`. | Valid Craftsky handle, valid non-Craftsky handle, invalid/missing handle. | Craftsky handle returns minimal resolve object; non-Craftsky/missing returns no usable target and handler uses `404 mention_not_found`. | `appview/internal/api/facet_test.go`, `app/test/shared/rich_text/facet_generator_test.dart` |
...
| UT-007 | FR-003, FR-008, BR-001 | AC-002, AC-006, AC-016 | Flutter exact mention resolver maps success and `mention_not_found` to facet generation behavior. | Dio success for Alice; 404 envelope for unknown. | Alice returns DID; unknown returns null/no mention facet; resolver is used during final generation only. | `app/test/shared/rich_text/facet_generator_test.dart`, `app/test/shared/rich_text/facet_suggestion_repository_test.dart` |
...
| UT-009 | FR-011, NFR-003, BR-003 | AC-010, AC-020; EC-003, EC-004 | Parse plain bio text with post facet generator token rules. | Dotted `@handle`, Unicode hashtags, HTTP/S URLs, bare domains, `mailto:`, malformed handles, URL fragment with `#tag`. | Supported tokens become deterministic non-overlapping ranges; bare domains normalize to HTTPS; non-HTTP/S or malformed tokens remain plain text and do not throw. | `app/test/profile/widgets/profile_bio_test.dart` or a shared parser test |
...
| IT-003 | FR-003, FR-004, FR-014, RULE-001 | AC-002, AC-006, AC-013, AC-018; EC-001, EC-005 | Exact mention resolve refreshes missing/stale cache and filters to Craftsky profiles. | Seed Craftsky profile for Alice, no Craftsky profile for Mallory, stale cache for Alice, fake directory resolver. | Resolve Alice, Mallory, and unknown handles. | Alice returns `{did, handle, isCraftskyProfile:true}` and refreshes cache; Mallory/unknown produce `404 mention_not_found`; stale entries are refreshed on successful resolution. | `appview/internal/api/facet_store_test.go`, `appview/internal/api/facet_test.go` |
...
| IT-005 | FR-015 | AC-019 | Bounded identity-cache backfill populates handles for existing Craftsky profiles without SQL migration network work. | Test DB has existing Craftsky profile rows and empty identity cache; fake bounded resolver records calls. | Run CLI/ops/bootstrap command or equivalent function with a limit. | Cache rows are inserted/updated for existing profiles; command respects limit/bounds; migration DDL contains no network resolution. | `appview/cmd/cli/*_test.go`, `appview/internal/api/identity_cache_store_test.go` |
...
| TD-005 | Profile bio plain text | `Knitting with @alice.craftsky.social #lace craftsky.social https://example.com mailto:x@y.example https://craftsky.social/#lace`. | AT-004, AT-005, UT-009, UT-010, IT-007, IT-008 |
...
| MAN-002 | FR-015 | Identity-cache backfill operator smoke. | Run the implemented bounded backfill command in a dev database with pre-existing Craftsky profiles. | Command reports bounded progress, does not run from SQL migration, and autocomplete can find backfilled handles. |
...
|---|---|---|---|---|
| GAP-001 | Over-limit requests may be clamped or rejected. | NFR-002 | Requirements allow either behavior for `limit > 25` as long as no more than 25 items are returned. | Coding plan should choose one behavior and tests should assert that exact choice. |
| GAP-002 | Exact CLI command name for identity-cache backfill is not locked. | FR-015 | Requirements specify a bounded CLI/ops/bootstrap path, not a concrete command name. | Coding plan should name the command and wire IT-005/MAN-002 to it. |
