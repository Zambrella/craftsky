...
Feature: Instagram integration availability
  Scenario: Meta-dependent work is unavailable without locking private controls
    ...
    And verified-link import create/list/delete, current suggestion review/dismiss/accept, existing-link status/disable/revoke, and notification preferences remain available when their own dependencies are healthy
...
### AT-005: Retain Handles Until Unlink And Deliver A Future-Match Digest
...
Feature: Future Instagram matches
  Scenario: Several retained following handles become discoverable together
    ...
    And AppView creates one actorless instagramMatch digest for Alice with current bounded count and activity/newness
    ...
    When Alice unlinks her Instagram account
    Then all of Alice's imports and retained handles are deleted
    And dependent pending suggestions and unsent notifications are invalidated
    And accepted PDS follows remain unchanged
```
...
| UT-012 | FR-020, FR-022 | AC-035–AC-037 | Decode type-discriminated notifications and infer actorless `instagramMatch` destinations in Flutter. | Existing social types; `type: instagramMatch`, `system: {count, countCapped}` with no `kind` or server destination; absent actor/source facts; malformed/unknown types; stale/current suggestions. | Known match requires no actor/URI/CID/rkey or route field, renders generic bounded copy, and Flutter targets the Instagram route from `type`; social types retain required facts; unknown types remain inert without relying on a second discriminator. | `app/test/notifications/models/notification_test.dart`, `app/test/notifications/services/notification_destination_inference_test.dart` |
...
| IT-010 | FR-018, FR-028, FR-030, RULE-009, RULE-010 | AC-026–AC-028, AC-031, AC-044, AC-048 | Enforce permanent-until-unlink import storage, additive support, export, reversible membership inactivation, and terminal/scoped purge. | Long-lived matched/unmatched following rows; multi-supported imports; verified unlink; departed/rejoined/terminal owner; accepted-follow sentinel; other private record retention boundaries. | Advance time, delete one import, unlink, export/purge, depart/rejoin/reactivate each import, terminal-delete identity, and scoped-delete link/import. | Import handles do not age out; other support survives source deletion; unlink deletes owner imports and dependent pending state; departure pauses; terminal/scoped purge is narrow; accepted follow remains. | `appview/internal/instagram/retention_test.go`, `appview/internal/instagram/account_data_test.go`, `appview/internal/instagram/account_store_test.go` |
...
| IT-012 | FR-020–FR-022, FR-030 | AC-034–AC-037, AC-048 | Extend notification schema/API/newness/preferences/push with exact type-discriminated payload constraints. | Existing seven social type fixtures; actorless `instagramMatch` fixtures without `kind`; malformed column/JSON combinations; unknown type; fixed-scope preference; member/policy invalidation. | Migrate; insert invalid/valid variants; activate/read/page/count/mark seen/patch/push/retract each type. | DB checks derive actor/source versus system-field requirements only from category/type; API JSON omits `kind` and cursor/activity ordering matches golden JSON; fixed scope rejects mutation; one bounded private push survives only while currently eligible; existing social types remain unchanged. | `appview/internal/api/instagram_notification_test.go`, `appview/internal/push/instagram_payload_test.go` |
...
| IT-016 | FR-023–FR-026, NFR-007 | AC-003, AC-005, AC-009, AC-014, AC-016–AC-018, AC-023–AC-027, AC-038, AC-048 | Render and use the typed Instagram page in every state. | Provider fakes for disabled/empty/loading/error/attempt/candidate/link/active and membership-inactive imports/suggestions/conflict; semantics tester. | Enter from Settings; copy/open/cancel/confirm/toggle/revoke; verify imports are hidden without a link; directly import manual/JSON entries; open Notification Settings; delete/reactivate imports; review/select-all/dismiss/accept/retry. | Localized accessible controls/disclosures match state; pending confirmation places the selector directly below the account, defaults to discovery allowed, updates its explanation for both options, and confirms the displayed value; import controls appear only for a verified link and have no preview/retention choice; a rejoined member can explicitly reactivate each paused import; candidate/reason copy is accurate; select-all is limited to reviewed eligible rows. | `app/test/instagram_migration/instagram_migration_page_test.dart`, `app/test/settings/settings_page_test.dart`, `app/test/router/instagram_route_test.dart` |
...
| REG-002 | Existing seven notification categories retain actor/source semantics, preference defaults, feed hydration, newness, retraction, and push behavior. | FR-020, FR-021 | AC-034–AC-037 | Run all AppView notification/index/push tests with `instagramMatch` added only through its actorless type path. |
...
| REG-011 | AppView and Flutter cannot silently diverge on Instagram states, fields, notification variants, success codes, or errors. | FR-002–FR-025, NFR-004 | AC-003–AC-005, AC-009, AC-014–AC-017, AC-023–AC-027, AC-032, AC-034–AC-040 | Run both consumers against TD-011 and fail on an unreviewed golden-fixture delta. |
...
| TD-008 | Notification matrix | Existing seven social categories; exact actorless `instagramMatch` payload without `kind`; unknown and malformed types; counts 1/99/100; five-minute boundary; active/retracted events; pending/retry/leased/sent deliveries; A/B account bindings. | AT-005, AT-007, UT-012–UT-014, IT-011, IT-012, IT-017, IT-021 |
...
| TD-011 | Shared wire golden corpus | Synthetic JSON for every §12.1 request/success/error; all attempt/link/import/suggestion/conflict enums; type-discriminated social and actorless notifications with no `kind`; optional/omitted fields; 20/50 pages; owned/foreign/absent/purged DELETE 204; callback 200/403/429 and Retry-After; repeated idempotent results. | IT-002, IT-005–IT-009, IT-012, IT-014, IT-021 |
...
- Suggested test order for implementation:
  ...
  9. `UT-005`, `IT-007`, `IT-008`: strict verified-link additive imports, list/detail/reactivate/delete, normalization, eligibility, limits, and paginated suggestions.
