...
| FR-012 | Functional | Must | `POST /v1/migrations/instagram/imports` shall accept a bounded strict schema containing source type, normalized usernames for accounts the member follows, and retention consent; it shall create a private import and return the following count/initial suggestions without accepting relationship direction, follower data, or raw archive fields. | Defines the minimal server boundary and avoids collecting unused follower graph data. | Design / User approval | AC-016–AC-019 |
...
| `POST /v1/migrations/instagram/imports` | `201`; request `{sourceType: manual | instagramJson, retainUnmatched, entries: [{username}]}`; response `{import, counts: {followingCount}, initialSuggestionCount}`. Direction and follower-specific fields are unknown fields and are rejected. | `400 invalid_request`; `413 request_too_large`; `422 invalid_instagram_import`; `429 rate_limited`. |
...
| `GET /v1/migrations/instagram/imports/{importId}` | `200`; `{importId, state, sourceType, retainUnmatched, retentionExpiresAt?, followingCount, createdAt}`. No handle list is returned. | `404 instagram_import_not_found`. |
...
| AC-014 | FR-008, FR-024, RULE-002, RULE-004 | Given a pending candidate, when Flutter shows the actual username followed by a selector defaulted to `Allow discovery`, then the selected option's explanation appears immediately below it. When the creating member explicitly confirms, the displayed discoverability value is applied and the link is finalized exactly once; a different DID or unconfirmed UI cannot finalize it. |
...
| Import and graph handles | Storage contains accounts-followed usernames only and has no direction or follower-count fields. With retention declined, unmatched following rows are deleted in the initial match transaction. A matched row remains only as support while its suggestion is pending. Source metadata then retains aggregate counts only. With consent, unmatched rows and source metadata expire together; membership inactivity does not extend expiry. Turning consent on after values were discarded cannot recover them; the member must create a new import. | Consented source: 12 months from creation/latest explicit renewal. Non-consented matched support: until suggestion terminal, at most 12 months from import creation. Aggregate-only metadata: until 90 days after its final suggestion becomes terminal, at most 12 months from creation. |
