...
- When viewing someone else's profile, show a capped mutual-follower preview such as `3 mutual followers`; tapping it opens a modal/bottom sheet with the list of mutuals.
- A stat that shows the account's age.
- Recent activity shown as `X posts in the last 7 days`, including all authored post rows but excluding repost interactions.
...
Decision / implication: Add profile summary fields to existing profile responses, add list endpoints for mutuals/followers/following, and update Flutter profile/settings UI. Recent activity counts Craftsky post rows but excludes repost interactions; account age is computed in Flutter from the existing Craftsky profile `createdAt` value.

## 4. Candidate Approaches
### Option A: Profile summary API (Recommended)
Summary: Extend existing profile responses with profile-summary fields and add paginated list endpoints for mutual followers, followers, and following. Update Flutter profile and settings UI to consume those AppView contracts, including a capped mutuals preview that opens a modal/bottom sheet for the full mutuals list.
...
## 11. Desired Behavior
Profile pages no longer display follower or following counts. Visitor profiles display mutual followers as a capped preview/count label, such as `3 mutual followers`, that opens a modal/bottom sheet with the mutuals list. Profile stats show account age using `Joined <age> ago` copy, recent activity as posts in the last 7 days, total posts, and the existing projects stat. Settings gives the signed-in user entry points to follower and following list pages without showing the counts on the settings page itself; each list page is ordered by recency and may show its own count.
...
| BR-002 | Business | Must | Visitor profiles must show viewer-specific mutual-follower context through a capped preview/count that opens a mutuals list modal or bottom sheet. | Helps viewers understand shared community connections without restoring popularity counts. | Prompt, Q1, review feedback | AC-002, AC-003, AC-004, AC-015 |
...
| FR-002 | Functional | Must | When the signed-in viewer opens someone else's profile, the profile page shall show a capped mutual-followers preview/count label for accounts the viewer follows who also follow the current profile. | Replaces public follower counts with viewer-specific relationship context. | Prompt, Q1, review feedback | AC-002, AC-003, AC-004, AC-015 |
...
| FR-004 | Functional | Must | The profile summary shall provide the account creation timestamp, and Flutter shall compute and render account age from that timestamp using `Joined <age> ago` copy. | Provides requested account-age stat while keeping age formatting a frontend concern. | Prompt, Q1, codebase, review feedback | AC-005 |
| FR-005 | Functional | Must | The profile summary shall expose and render recent activity as the number of Craftsky post rows authored by the account in the last 7 days. | Provides requested recent-activity stat across authored post rows while excluding repost interactions. | Prompt, Q1, codebase, review feedback | AC-006 |
...
| FR-012 | Functional | Must | The API shall continue making `followerCount` and `followingCount` available in profile-related responses, but Flutter shall not display those counts on profile pages or on the settings entry page. | Keeps graph totals available to clients while satisfying the UX change. | Discovery, review feedback | AC-001, AC-008, AC-016 |
| FR-013 | Functional | Must | Tapping the mutual-follower preview/count on a visitor profile shall open a modal or bottom sheet containing the mutual followers list, with access to view beyond the capped preview. | Makes mutual context explorable without crowding the profile header. | Review feedback | AC-015 |
...
| RULE-003 | Business rule | Must | Recent activity counts authored rows in `craftsky_posts` within the trailing 7-day window, including root posts and replies/comments; repost interactions, likes, and follows do not count. | Matches confirmed direction, review feedback, and current data model. | Prompt, Q1, codebase, review feedback | AC-006 |
...
| AC-002 | BR-002, FR-002 | Given a signed-in viewer opens another Craftsky profile with at least one mutual follower, when the profile loads, then the mutual-follower section displays a capped preview/count label such as `3 mutual followers`. |
...
| AC-005 | FR-004, RULE-002 | Given a profile response includes a Craftsky creation timestamp, when the profile renders, then Flutter computes the age and displays it using `Joined <age> ago` copy. |
| AC-006 | FR-005, RULE-003 | Given an account authored Craftsky post rows inside and outside the trailing 7-day window and has repost interactions, when the profile summary is fetched, then `posts in the last 7 days` includes only authored `craftsky_posts` rows within the trailing 7-day window and excludes repost interactions. |
...
| AC-015 | BR-002, FR-002, FR-013 | Given a visitor profile shows a mutual-follower preview/count, when the viewer taps it or its `view all` affordance, then a modal or bottom sheet opens with the mutual followers list. |
| AC-016 | FR-012 | Given a client requests profile-related API data, when AppView returns the response, then follower and following counts remain available in the API contract even though the Flutter profile/settings entry UI does not display them. |
| AC-017 | FR-014 | Given a user opens an empty follower or following list page, when the page renders, then it shows a simple empty-state message such as `You are not following anyone`; given a visitor profile has zero mutual followers, then no mutual-follower section is shown. |

## 14. Edge Cases
| ID | Case | Expected Behavior | Requirement IDs |
|---|---|---|---|
| EC-001 | No mutual followers | Visitor profile shows no mutual-follower section and does not show follower/following counts. | FR-001, FR-002, FR-014 |
...
| EC-006 | Follower/following list is empty | Settings list page shows a simple empty-state message, not an error or discovery CTA. | FR-008, FR-009, FR-014 |
...
- New fields:
  - Expected additive profile summary fields include total post count, posts-in-last-7-days count, mutual-follower count, and a capped mutual-follower preview. Exact JSON names should use camelCase.
...
- UI:
  ...
  - Visitor profiles add a capped mutual-follower preview/count with a `view all` action that opens a modal or bottom sheet.
...
| RISK-003 | "All types of posts" could be interpreted differently by future contributors. | Recent activity could accidentally include repost interactions. | Requirements explicitly exclude repost interactions and count only authored `craftsky_posts` rows. |
...
| ASM-002 | Recent activity counts root posts and replies/comments in `craftsky_posts`; repost interactions, likes, and follows do not count. | Requirements must change if product later wants reposts or other interactions included. |
| ASM-003 | Mutual-follower section is absent when there are no mutuals; when mutuals exist, the profile shows a capped preview/count with a `view all` affordance. | UI requirements must change if product later wants an explicit zero-mutuals message. |
...
Reviewer:
Date:
Notes: Plannotator feedback has been incorporated. This is a user-visible full-stack change touching AppView API shape, graph queries, Flutter profile UI, Flutter settings navigation, and tests. A second review is optional before test design.
...
- Must-cover requirement IDs:
  ...
  - FR-001, FR-002, FR-003, FR-004, FR-005, FR-006, FR-007, FR-008, FR-009, FR-010, FR-012, FR-013, FR-014
  - NFR-001
...
- Suggested test levels:
  ...
  - Flutter widget tests for profile stats, mutual-follower preview/modal, settings entries without counts, list empty states, and absence of follower/following counts on profile pages.
