...
## 11. Desired Behavior
Profile pages no longer display follower or following counts. Visitor profiles display mutual followers as uncapped clickable text, such as `12 mutual followers`, that opens a 90%-height bottom sheet with the paginated mutuals list. Profile stats show Craftsky account age using `Joined <age> ago` copy, recent activity as top-level posts in the last 7 days, total top-level posts, and total projects from response data. Non-Craftsky profiles do not show account age. Settings gives the signed-in user entry points to follower and following list pages without showing the counts on the settings page itself; each list page is ordered by recency and shows its count in the app bar title. The settings following list includes only followed accounts that have Craftsky profiles; non-Craftsky followed accounts are excluded.
...
| FR-009 | Functional | Must | Tapping following in settings shall show Craftsky accounts the signed-in user follows ordered by most recent follow first, with the following count shown in the list page app bar title. Non-Craftsky followed accounts shall be excluded. | Satisfies list order requirement and keeps counts one tap deeper than settings while keeping the in-app following list scoped to Craftsky profiles. | Prompt, Q2, discovery, user clarification | AC-010 |
...
| AC-010 | BR-003, FR-009, RULE-004 | Given the signed-in user follows Craftsky and non-Craftsky accounts at different times, when they tap the following settings entry, then only followed Craftsky accounts appear, those rows are ordered newest follow first, and the following count in the app bar title counts only those Craftsky rows. |
...
| EC-008 | User follows only non-Craftsky accounts | Following settings list shows `You are not following anyone` because no followed Craftsky profiles are in scope. | FR-009, FR-014 |
