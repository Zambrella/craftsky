...
- Verification to rerun: After each refactor, run the focused notification tests and targeted analysis. After the runtime/registration consolidation, rerun linked auth/router/app lifecycle tests, canonical `just app-test`, `just app-analyze`, `git diff --check`, and the relevant AppView notification tests. Keep manual delivery checks separately gated by `PUSH_ENABLED=false` before and after the bounded session.
