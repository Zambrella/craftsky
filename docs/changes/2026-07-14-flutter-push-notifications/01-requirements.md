...
- Constraints discovered:
  ...
  - AppView configuration defaults push sending to disabled. The repository development launcher may enable it automatically when readable external ADC is available, and must otherwise start normally with sending disabled.
...
| FR-026 | Functional | Must | Repository-managed development startup shall enable real FCM sending when readable external ADC and the Firebase project ID are available, and shall start normally with sending disabled when ADC is absent. Credentials shall remain outside the repository; production enablement remains explicitly deployment-managed. | Makes push behavior realistic for configured developers without making credentials a prerequisite for ordinary development. | User decision / existing AppView configuration | AC-030 |
...
| AC-030 | FR-026 | Given repository-managed development startup, when readable ADC is available, then AppView receives the credential as a read-only secret and real FCM sending is enabled; when ADC is absent, AppView starts normally with real sending disabled. |
...
| EC-021 | Development AppView starts with or without local ADC. | Readable ADC enables real FCM sending; absent ADC produces a clear status message and leaves sending disabled without blocking startup. | FR-026 |
...
| RISK-013 | One Firebase project is shared across environments. | A development AppView can send real alerts to tokens registered in its local database. | Keep credentials external, print the resolved push state at startup, scope manual checks to intended local device/account registrations, and disable automatically when ADC is absent. |
...
| ASM-009 | A single Firebase project is acceptable for every build environment when development sending requires external ADC, reports its resolved state, and targets intentional registrations in the local database. | Stronger isolation would require another Firebase project or native build variants, both explicitly excluded. |
