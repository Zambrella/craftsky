...
Automated Flutter tests must not initialize Firebase, contact FCM, request real OS permission, open native settings, or depend on physical-device state. One physical Android device and one physical iOS device remain necessary for provider delivery, native permission, background/terminated presentation, sound/vibration, token lifecycle, and settings-recovery checks. Repository-managed development startup enables sending when readable external ADC is available and otherwise starts normally with sending disabled; manual delivery checks must confirm the resolved state and intended local device/account before generating events.
...
| IT-015 | FR-026 | AC-030 | Validate credential-aware development push selection. | Development helper with readable ADC and with no readable ADC; enabled AppView config with/without project ID | Resolve Compose configuration and load AppView configuration | Readable ADC selects the read-only secret override and enables push; absent ADC selects the base disabled configuration; enabled AppView config still requires a project ID. | `scripts/compose-dev`, `docker-compose.push.yml`, `appview/internal/app/push_config_test.go` |
...
|---|---|---|---|---|---|
| MAN-001 | BR-001, BR-002, FR-001, FR-007, FR-008, FR-023, RULE-002 | AC-001, AC-006, AC-007, AC-027 | Android physical-device delivery and open matrix | Confirm development startup reports push enabled and record the intended device/account. Install a build registered as `social.craftsky.app`, grant permission, deliver foreground/background/terminated messages without an explicit Android `channel_id`, tap banner/OS notifications, and inspect the selected channel. | Foreground shows one silent/non-vibrating Craftsky banner and refreshes data; background/terminated uses the manifest-bound single “CraftSky notifications” channel with standard sound/vibration rather than an FCM fallback channel; every tap resolves through AppView to the authorized target/fallback. |
| MAN-002 | BR-001, BR-002, FR-001, FR-007, FR-008, FR-023, FR-025, RULE-002 | AC-001, AC-006, AC-007, AC-027 | iOS physical-device delivery and open matrix | Upload/configure the APNs authentication key in Firebase, confirm development startup reports push enabled, record the intended device/account, install the `social.craftsky.app` build, grant alert/sound permission, deliver foreground/background/terminated messages, and inspect app-icon behavior. | Foreground banner is silent and does not update app-icon badge; background/terminated alerts use default sound; taps resolve safely; no badge authorization or app-icon count is used. |
...
| MAN-005 | FR-026 | AC-030 | Credential-aware development sender selection | Start through the repository helper once without readable ADC and once with readable ADC, inspecting the reported state and resolved Compose configuration without sending a message. | Missing ADC starts AppView with push disabled; readable ADC enables push and mounts only the external credential as a read-only secret. |
