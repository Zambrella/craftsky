...
Use a test-first path that starts at the AppView read contract, then adds Flutter data/model/provider coverage, then widget-level user workflows. Backend tests should pin the comment-section API shape, top-level grouping/sorting/pagination, focused reply branch inclusion, bounded child slices, child reply pagination, and `/thread` removal. Flutter tests should verify route parsing, state transitions, de-duplication, expansion/collapse controls, new-reply scroll/focus behavior, and the two-level visual cap.
...
Feature: Reply deep links
  Scenario: Opening a focused reply link
    Given a root post has an indexed reply
    ...
    And the focused reply is identified in the loaded comment state
    And the focused reply is scrolled or highlighted into view
```
...
Feature: Reply deep links
  Scenario: Focused nested reply is outside the first page
    Given a root post has more than 10 top-level replies
    And the focused reply belongs to a top-level branch outside the first top-level page
    And the focused reply is outside the first 10 child replies for that branch
    When the focused route is opened
    Then the focused top-level branch is included without loading all intermediate top-level pages
    And a bounded child slice containing the focused reply is displayed
...
Feature: Root post comment section
  Scenario: Opening a post without focus
    Given a root post has top-level replies and child replies
    When the user opens /posts/{rootDid}/{rootRkey} without a focus parameter
    Then the root post is visible
    And top-level replies are visible below it
    And child replies are not visible until the user expands a branch
```
...
Feature: Top-level reply pagination
  Scenario: Scrolling loads more top-level replies
    Given a root post has more than 10 top-level replies
    And the first 10 replies are displayed
    When the user scrolls near the end of the top-level reply list
    Then the app requests the next page using the current cursor and sort
    And up to 10 additional top-level replies are appended
...
Feature: Child reply controls
  Scenario: Managing child replies under a top-level reply
    Given a top-level reply has more than 10 child replies
    And child replies are not loaded
    When the user taps "view replies"
    Then the first 10 child replies are shown oldest-first
    And the branch control changes to "hide replies"
    And a "load more" control is shown
    When the user taps "load more"
    Then up to 10 more child replies are appended oldest-first
    When the user taps "hide replies"
    Then the child reply list collapses without deleting reply data
```
...
Feature: Reply ordering
  Scenario Outline: Selecting top-level reply ordering
    Given the viewer and other users have authored top-level replies
    When the user selects <sort> from the ordering dropdown
    Then viewer-authored top-level replies appear before non-viewer replies
    And replies inside each group use <effectiveOrder>
    And expanded second-level replies remain oldest-first
...
Feature: Reply creation feedback
  Scenario: Creating a top-level reply
    Given the user is viewing a root post with any top-level sort selected
    When the user creates a top-level reply
    Then the new reply appears in the viewer-authored top-level group
...
```gherkin
Feature: Reply creation feedback
  Scenario: Replying to a second-level reply
    Given a second-level reply is visible
    When the user taps reply on that second-level reply
    ...
    And the created reply is displayed in the nearest top-level branch's second-level list
...
Feature: Two-level visual cap
  Scenario: Deep backend reply is displayed from focus
    ...
    And the nearest top-level ancestor is displayed as a top-level reply
    And the focused deep reply is displayed in the second-level list
...
|---|---|---|---|---|---|---|
| UT-001 | FR-007, FR-008, RULE-005, RULE-006 | AC-007, AC-020 | Sort comparator groups viewer-authored top-level replies first and maps `follows` to oldest. | Mixed viewer/non-viewer replies with timestamps and sort values. | Viewer group first; `oldest`/`follows` oldest-first; `newest` newest-first inside each group. | `app/test/feed/models/post_comment_section_state_test.dart` or backend pure sort helper test. |
| UT-002 | FR-007, FR-010, FR-011, RULE-004 | AC-010 | Child reply ordering ignores top-level sort. | Child replies with shuffled timestamps under each top-level sort. | Child replies render oldest-first for every top-level sort. | `app/test/feed/models/post_comment_section_state_test.dart` |
| UT-003 | FR-013, FR-014, RULE-003 | AC-014 | Tree-flattening logic maps deeper replies to nearest top-level branch. | Root, top-level, second-level, third-level reply chain. | Output contains only top-level and second-level visual nodes. | `app/test/feed/models/post_comment_section_state_test.dart` |
| UT-004 | FR-005, RULE-002 | AC-004 | Initial render state excludes child reply lists. | Root post response with top-level replies carrying child counts. | State contains root and top-level replies; branch expansion states are collapsed. | `app/test/feed/providers/post_comment_section_provider_test.dart` |
...
| UT-007 | FR-002 | AC-008 | Client model decodes comment-section response shape. | JSON with `post`, top-level `items`, `cursor`, `sort`, focused metadata. | Model fields decode with camelCase keys and optional cursor/focus fields. | `app/test/feed/models/post_comment_section_test.dart` |
...
| UT-009 | FR-011, NFR-002 | AC-009 | Child load-more state is per top-level branch. | Two expanded branches with independent cursors. | Loading one branch does not mutate the other branch cursor/items. | `app/test/feed/providers/post_comment_section_provider_test.dart` |
| UT-010 | FR-017 | AC-012 | New nested reply insertion targets relevant second-level branch. | Created reply whose parent is a child/deeper reply. | Reply appears in nearest top-level branch second-level list. | `app/test/feed/models/post_comment_section_state_test.dart` |
...
|---|---|---|---|---|---|---|---|
| IT-001 | FR-002, RULE-002 | AC-004, AC-008 | Comment-section endpoint returns root and top-level replies only. | Seed root, top-level replies, and child replies in Postgres fixture. | GET comment-section endpoint without focus. | Response includes root and top-level page; no child reply items are expanded by default. | `appview/internal/api/post_test.go` handler test + `post_store_test.go`. |
...
| IT-003 | BR-001, FR-004 | AC-002, AC-003 | Focused top-level ancestor outside first page is included. | Seed >10 top-level replies; focused branch is after first page. | GET first page with focus. | Focused branch included without intermediate top-level pages. | `appview/internal/api/post_store_test.go`. |
| IT-004 | FR-006, NFR-002 | AC-005, AC-009 | Top-level replies page in chunks of 10 with opaque cursor. | Seed 12 top-level replies. | Request limit 10, then cursor. | Page 1 has 10, page 2 has 2, cursor omitted on final page. | `appview/internal/api/post_store_test.go`. |
| IT-005 | FR-007, FR-008, RULE-005, RULE-006 | AC-007, AC-020 | Top-level sorting and viewer grouping. | Seed viewer-authored and other top-level replies with different timestamps. | Request `sort=oldest`, `sort=newest`, `sort=follows`. | Viewer-authored group first; sort applies within groups; follows equals oldest. | `appview/internal/api/post_store_test.go`. |
| IT-006 | FR-004, RULE-007 | AC-003, AC-021 | Focused child outside first child page uses bounded focused slice. | Seed top-level reply with >10 child replies and focus child after first 10. | GET comment section with focus. | Expanded branch contains target in bounded slice and exposes child pagination state. | `appview/internal/api/post_store_test.go`. |
| IT-007 | FR-010, FR-011, RULE-004 | AC-006, AC-009, AC-010 | Direct child replies load 10 at a time oldest-first. | Seed top-level reply with 12 direct children. | GET child replies with limit 10, then cursor. | First 10 and next 2 are oldest-first; no grandchildren in direct child page except flattened focus/new-create cases handled elsewhere. | `appview/internal/api/post_store_test.go`, update existing `ListDirectReplies` tests. |
| IT-008 | FR-014, RULE-001 | AC-015, AC-019 | Creating reply preserves actual parent and root refs. | Existing create-post handler with reply target parent/root refs. | POST reply to second-level target. | PDS record body contains actual parent ref and root ref; no lexicon change needed. | `appview/internal/api/post_test.go`, existing create reply tests. |
...
| IT-011 | NFR-002, RULE-007 | AC-021 | Focused child slice is bounded. | Seed many child replies before focused target. | Request focus. | Response does not include every earlier child reply and remains within documented bound. | `appview/internal/api/post_store_test.go`. |
...
| REG-005 | Existing direct replies endpoint pagination oldest-first remains valid for child reply loading if reused. | FR-010, FR-011, RULE-004 | Preserve/update `TestPostStore_ListDirectReplies_PaginatesOpaqueCursorOldestFirst` with limit 10 expectations. |

## 7. Test Data

| ID | Purpose | Data | Used By |
|---|---|---|---|
| TD-001 | Basic root comment section | Root post by Alice; 3 top-level replies by Bob/Carol/Dave; each has reply counts. | AT-003, IT-001, UT-004 |
| TD-002 | Top-level pagination | Root post with 12 top-level replies, timestamps one minute apart. | AT-004, IT-004 |
| TD-003 | Viewer grouping and sort | Viewer-authored top-level replies at early/late timestamps plus non-viewer replies at interleaved timestamps. | AT-006, AT-007, IT-005, UT-001, UT-011 |
| TD-004 | Child reply pagination | One top-level reply with 12 direct child replies oldest-first. | AT-005, IT-007, UT-009 |
| TD-005 | Focus outside pages | >10 top-level replies; focused branch outside page 1; branch has >10 children; focused child after first 10. | AT-002, IT-003, IT-006, IT-011 |
| TD-006 | Deeper backend chain | Root -> top-level -> second-level -> third-level target, with actual root/parent refs. | AT-008, AT-009, IT-008, UT-003, UT-010 |
...
| MAN-002 | FR-006, FR-011 | Scroll and load-more UX feel. | Use a seeded/development account with >10 top-level replies and >10 child replies. Scroll the root list and use child load more. | Top-level lazy loading and child action loading feel predictable, with no duplicate or disappearing replies. |
...
| GAP-002 | Exact visual treatment for flattened deeper reply parent context remains non-blocking. | FR-014, FR-015, RULE-003 | Requirements mandate composer mention but leave optional “replying to” label undecided. | Test only the required mention and two-level cap now; add tests if product chooses additional context labeling. |
...
- Recommended first failing test for implementation: `IT-001` for the new AppView comment-section read surface returning root post plus top-level replies only. This pins the API contract before Flutter work depends on it.
- Suggested test order for implementation:
  1. `IT-001`, `IT-004`, `IT-005` — backend comment-section contract, pagination, sort/viewer grouping.
  2. `IT-002`, `IT-003`, `IT-006`, `IT-011` — focus handling and bounded focused child slices.
  3. `IT-007`, `IT-008`, `IT-009`, `IT-010` — child loading, parent refs, route removal, API conventions.
  4. `UT-007`, `UT-006`, `UT-001` through `UT-011` — Flutter models/providers/state merging.
