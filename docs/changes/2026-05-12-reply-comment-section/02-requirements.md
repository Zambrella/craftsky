...
- NG-006: Permanently grouping all viewer-authored comments at the top; only newly-created top-level replies receive a temporary post-confirmation pin in this change.
...
| FR-004 | Functional | Must | Opening a focused reply link shall include and render the focused reply branch even when the focused reply would not appear in the first top-level reply page. | Deep links must be reliable independent of pagination. | Discovery Q3 | AC-002, AC-003, AC-014 |
...
| FR-007 | Functional | Must | Top-level reply ordering shall support `oldest`, `newest`, and `follows`, with ordering applied only to top-level replies. | Sorting child replies would conflict with conversation order and was explicitly excluded. | Discovery Q2/Q6 | AC-007, AC-010 |
...
| FR-016 | Functional | Must | A newly-created top-level reply shall be temporarily pinned at the top of the top-level list and scrolled into view, regardless of the selected sort. | The user chose temporary pinning over respecting sort or switching sort. | Discovery Q4 | AC-011 |
...
| NFR-003 | Non-functional | Should | Focus and lazy-loading behavior should avoid duplicate rendered replies when a focused branch overlaps with a loaded page or a newly-created pinned reply. | Prevents confusing duplicate comments. | Requirements analysis | AC-018 |
...
| RULE-006 | Business rule | Must | Temporary pinning applies to newly-created top-level replies only; permanent “my comments first” grouping is out of scope unless separately decided. | Aligns with chronological/no-ranking product principle. | Discovery review note | AC-011 |
...
| AC-002 | BR-001, FR-004 | Given the focused reply is outside the first top-level reply page, when the comment section loads, then the focused reply branch is included without requiring the user to scroll/load intermediate pages first. |
| AC-003 | BR-001, FR-003, FR-004 | Given the focused reply is nested under a top-level reply, when the comment section loads, then the relevant branch is expanded and the focused reply is scrolled or otherwise brought into view. |
...
| AC-007 | BR-003, FR-007, FR-008, RULE-002, RULE-005 | Given the ordering dropdown is used, when `oldest`, `newest`, or `follows` is selected, then top-level replies are ordered oldest-first, newest-first, or oldest-first respectively, and second-level reply order is unaffected. |
...
| AC-011 | BR-004, FR-016, RULE-006 | Given a top-level reply is successfully created, when the comment section updates, then the new reply appears temporarily pinned at the top of the top-level list and is scrolled into view without changing the selected sort. |
...
| AC-018 | NFR-003 | Given a focused reply or newly-created pinned reply also appears in a loaded page, when the list renders, then the reply appears only once in the visible comment section. |
...
| EC-008 | Newly-created top-level reply is later encountered in a fetched page. | De-duplicate against the temporary pinned item. | FR-016, NFR-003 |
...
- UI:
  ...
  - Add temporary pinned new top-level reply behavior and scroll/focus behavior for new/focused replies.
- API:
...
| RISK-003 | Temporary pinning plus pagination may duplicate comments. | Confusing UI and incorrect perceived counts. | Require visible de-duplication and test overlap cases. |
...
| ASM-003 | Temporary pinned top-level replies remain pinned until refresh, route exit, sort change, or de-duplication with a fetched page; no permanent “my comments first” behavior is included. | Product may need an additional rule if viewer-authored comments should always be grouped. |
...
- Suggested test levels:
  - Backend handler/store tests for comment-section response, sorting, pagination, focus inclusion, child reply loading, and `/thread` removal.
  - API contract tests for camelCase JSON, standard errors, opaque cursor behavior, and invalid focus/cursor handling.
  - Flutter provider/state tests for top-level lazy loading, per-branch expansion/collapse, de-duplication, sort changes, and new reply pinning.
...
- Blocking open questions: None for test design if `ASM-001` is accepted. Non-blocking product/design questions remain around permanent viewer-authored comment grouping and exact flattened-reply context labeling.
