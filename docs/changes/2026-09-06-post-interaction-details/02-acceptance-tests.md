...
| IT-006 | FR-009, NFR-002 | AC-015, AC-016 | Verify identifier, target, limit, and cursor failures. | Handler with resolvable/missing/hidden/muted targets, response targets, and valid target/type-bound cursors. | Send malformed DID/rkey, invalid limits, malformed cursor, wrong-target cursor, wrong-type cursor, and response-target Reposts/Quotes requests. | Missing/hidden and response-target Reposts/Quotes return `404 post_not_found`; muted targets remain eligible; cursor misuse is `400 invalid_cursor`; no items leak. | `appview/internal/api/post_interaction_handler_test.go` |
