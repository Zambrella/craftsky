...
| UT-013 | FR-031 | AC-035, AC-040 | Manage fresh-authorization retry attempts. | Poll recovery, ambiguous upload, explicit Retry, interruption, owner change, `already_exists`, and verified blob rejected by the PDS as missing | Poll recovery may keep the current job; upload/manual/interrupted retry discards old credentials and obtains fresh authorization; stale missing-blob recovery runs at most once with a valid no-op MP4 `free` box, surfaces a repeated failure, and does not alter normal upload bytes. | `app/test/feed/composer/video_publish_attempt_test.dart`, `app/test/feed/composer/video_publication_coordinator_test.dart`, `app/test/feed/data/video_service_transport_security_test.dart` |
...
| IT-003 | FR-004, FR-005, FR-006, FR-031, NFR-001 | AC-004, AC-005, AC-006, AC-013, AC-040 | Poll directly and independently verify submitted completion. | Fake public video service with Alice/Bob, incomplete, matching/mismatched, `already_exists`, outage, and downstream PDS `BlobNotFound` results | Flutter polls; then POST video proof through AppView | Flutter normalizes public states; AppView accepts only completed owner DID plus exact blob, discloses no foreign details, fails closed on verification outage, and maps only a verified video's downstream `BlobNotFound` to bounded `video_blob_missing`. | `app/test/feed/data/video_service_client_test.dart`, `appview/internal/video/completion_verifier_test.go`, `appview/internal/api/post_test.go` |
