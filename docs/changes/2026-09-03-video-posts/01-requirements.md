...
- Constraints discovered:
  ...
  - The standard Bluesky AppView builds `app.bsky.embed.video#view` playback
    ...
    playlist and thumbnail templates. Current playback uses
    `https://video.bsky.app/watch/{did}/{cid}/playlist.m3u8`, while thumbnails
    use `https://video.cdn.bsky.app/hls/{did}/{cid}/thumbnail.jpg`.
...
| FR-011 | Functional | Must | AppView shall derive the processed HLS playlist and optional thumbnail from percent-encoded author DID and video CID through configurable URL templates. Defaults shall match the selected Bluesky service's current `https://video.bsky.app/watch/{did}/{cid}/playlist.m3u8` playlist and `https://video.cdn.bsky.app/hls/{did}/{cid}/thumbnail.jpg` thumbnail contracts. No raw PDS MP4 source is exposed in this release. | Locks the confirmed HLS-only playback boundary while isolating CDN contract changes. | Requirements grilling, Bluesky AppView source and live response | AC-010, AC-011 |
...
| FR-017 | Functional | Must | Flutter shall expose provided video alt text through accessibility semantics without rendering it as visible post-card copy. | Preserves standard embed accessibility metadata without duplicating descriptive text in the visual layout. | `app.bsky.embed.video`, requirements grilling | AC-020, AC-041 |
...
| FR-031 | Functional | Must | An ambiguous upload failure or explicit Retry shall obtain a fresh service JWT and restart upload/processing; Craftsky shall not invent an unsupported idempotency key, and may reuse a BlobRef only when `already_exists` returns it and AppView verifies it. If the PDS then rejects that verified video with `BlobNotFound`, AppView shall return bounded `video_blob_missing`. Flutter may retry exactly once with fresh authorization and a standards-compliant no-op MP4 `free` box, but must surface a repeated failure without looping. Normal uploads remain byte-for-byte unchanged. | Matches the documented video-service contract while bounding an attempted recovery from stale service deduplication metadata. Live testing shows that Bluesky can still produce the same missing output CID after this retry. | Bluesky docs, AT Protocol blob lifecycle, user amendment | AC-035, AC-040 |
...
| AC-040 | FR-031 | Given an upload response is ambiguous, when publication retries, then Flutter requests fresh authorization and restarts the upload; if the service returns `already_exists` with a blob, AppView still independently verifies the owner and exact blob before publication. If the PDS rejects that verified blob as missing, AppView returns only bounded `video_blob_missing`; Flutter keeps the mutation loading, obtains fresh authorization, attempts one retry with a valid no-op MP4 `free` box, and surfaces a repeated failure without looping or mutating normal uploads. |
...
| RISK-013 | Cancellation or app interruption can leave remote processed blobs unreferenced, while video-service deduplication metadata can outlive the temporary PDS blob | A later attempt to post the same video can repeatedly receive a CID the PDS has already collected and fail with `BlobNotFound`. Live testing confirmed that a metadata-only MP4 `free` box retry can run a fresh processing job but still produce the same missing output CID. Selecting a different video works. | Do not delete PDS blobs; use verified `already_exists` normally, map a PDS `BlobNotFound` to bounded `video_blob_missing`, preserve the composer selection, and treat reliable same-video recovery as an unresolved upstream integration issue. Do not add client-side transcoding solely as a cache-bypass workaround. |
...
- [ ] Known upstream integration issue, confirmed by live testing on 2026-09-07:
  posting the same previously processed video can repeatedly fail with PDS
  `BlobNotFound`. A fresh Bluesky processing job created from a metadata-only MP4
  change can still return the same missing output CID. Selecting a different
  video succeeds. There is currently no reliable same-video recovery in
  Craftsky, and client-side transcoding is not an approved workaround.
