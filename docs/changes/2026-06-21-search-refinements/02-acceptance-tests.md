...
Feature: Submitted search result tabs
  Scenario: Four result categories fetch independently with relevance-first post and project results
    ...
    And post/project text search defaults to relevance score descending, then createdAt descending, then URI descending
```
...
Feature: Project browsing boundary
  Scenario: Project filters are served by /v1/projects rather than search UI endpoints
    ...
    And rich browse filters sent to `/v1/search/projects` are rejected with the standard validation error
```
...
Feature: AppView API conventions
  Scenario: Search, suggestion, and project endpoints use existing authenticated /v1 behavior
    ...
    When the endpoint is called with empty or whitespace-only required queries, invalid limits, invalid cursors, malformed tags, or unsupported parameters
...
| IT-005 | FR-006, FR-007, RULE-006, RULE-008 | AC-006, AC-007, AC-018 | Submitted post/project search is relevance-first and disjoint. | Seed regular post, project title/material matches, newer weak match, equal-score matches, hidden/reply rows. | Call `/v1/search/posts?q=alpaca` and `/v1/search/projects?q=alpaca` without sort override. | Posts returns only non-project top-level matches; Projects returns only project posts; ranking is relevance score descending, then `createdAt` descending, then URI descending. | `appview/internal/api/search_store_test.go` |
...
| IT-009 | FR-010, RULE-005 | AC-009, AC-019 | `/v1/search/projects` is text-search-only after filters move to `/v1/projects`. | Use request parser/route with rich browse filter params. | Call `/v1/search/projects?q=sock&craftType=...&material=...`. | Unsupported browse filters are rejected with the standard validation error; text-only Projects search remains available. | `appview/internal/api/search_request_test.go`, `appview/internal/routes/routes_test.go` |
...
| TD-003 | Submitted text-search relevance and disjoint tabs | Regular post text match, project title/material match, newer weak match, equal-score matches with controlled `createdAt` and URI values, project post that also matches post text, hidden/reply rows. | AT-004, UT-006, IT-005, REG-003 |
