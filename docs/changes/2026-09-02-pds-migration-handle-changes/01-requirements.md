...
| FR-001 | Functional | Must | At the shared OAuth-session coordinator, before each authenticated PDS effect uses a persisted parent, the AppView shall perform uncached DID resolution and verify that the parent's PDS and authorization server remain current. Validated effect-time OAuth metadata may use the bounded origin-keyed cache defined by FR-032. | This provides one simple mandatory safety boundary without caching mutable DID-to-PDS authority or distributing checks across handlers. | Backend audit; user answer | AC-007, AC-009, AC-010, AC-036, AC-056 |
...
| FR-032 | Functional | Must | For repeated ordinary authenticated PDS operations, Craftsky shall cache only successfully validated protected-resource and authorization-server metadata by canonical origin for a fixed five-minute insertion TTL and bounded capacity, while resolving the DID uncached before every operation. OAuth start, registration, and callback verification shall bypass this cache. Errors, cancellations, unsafe or malformed metadata, and non-success responses shall not be cached; concurrent same-key misses shall be coalesced. | This removes two steady-state metadata round trips without hiding a DID move to a different PDS or weakening the callback migration race boundary. | Approved performance plan; user answer | AC-056 |
...
| AC-056 | FR-001, FR-032, NFR-002, NFR-005 | Given repeated ordinary authenticated operations for DID D at unchanged PDS A, when the first and subsequent operations run within five minutes, then every operation resolves D uncached, only the first fetches A's protected-resource and authorization-server metadata, and concurrent misses coalesce. Given D moves to PDS B, the next operation detects B immediately regardless of cache warmth. OAuth start, registration, and callback checks remain fresh; failed or unsafe metadata is not retained; cache telemetry is bounded and secret-free. |
...
- New fields:
  - No persisted OAuth-authority cache, signing-key fingerprint, handle-status field, or migration-specific client state is required. A bounded process-local effect-time OAuth metadata cache is permitted by FR-032.
...
- Authentication:
  ...
  - Ordinary login callback completion must verify current authority fully fresh. Every authenticated PDS effect must resolve current DID-to-PDS authority uncached and may reuse only bounded validated OAuth metadata under FR-032.
...
|---|---|---|---|
| RISK-001 | Uncached DID authority resolution adds latency or is unavailable when a protected effect starts; cached metadata can delay detection of an issuer-only change at an unchanged PDS. | Writes can be slower or fail transiently, while an issuer-only transition may remain unseen for at most five minutes. | Keep DID resolution fresh, cache only validated metadata by origin with fixed insertion expiry and bounded capacity, and keep OAuth callbacks fully fresh. |
