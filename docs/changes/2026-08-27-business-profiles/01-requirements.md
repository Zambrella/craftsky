...
Decision / implication: Uppercase codes recognized by `golang.org/x/text/currency.ParseISO` use `currency.Standard.Rounding` for minor-unit scale. `XXX` (no currency) and `XTS` (testing) remain unsupported, but the application does not independently distinguish current, withdrawn, fund, or metal designators. Currency recognition follows the pinned module version and changes through ordinary reviewed dependency upgrades. The API returns only seller-authored amount/currency data and adds no disclaimer or commerce-guarantee fields.
...
| FR-006 | Functional | Must | Declaration location shall reference the CID-pinned `community.lexicon.location.address`; Craftsky mutation/response shape contains only country and locality, accepts only two-letter countries recognized as non-private countries by `golang.org/x/text/language`, canonicalizes case-insensitive input to uppercase, and bounds locality to 100 graphemes/1000 bytes. Events have no structured location field. | Reuses the community shape while minimizing hydrated data. | User interview, Lexicon rules, Document review | AC-012, AC-013 |
...
| FR-010 | Functional | Must | Optional product price shall use an uppercase code recognized by `golang.org/x/text/currency`, excluding `XXX` and `XTS`, and a canonical nonnegative decimal: integer `0` or 1-12 digits without a leading zero; optional fraction of 1 through the currency's standard rounding scale ending nonzero; no fraction for zero-scale currencies. Signs, exponents, separators, trailing fractional zeros, unknown currency, excess precision, and noncanonical zero are invalid. | Makes displayed prices exact and bounded while delegating currency maintenance to a maintained Go package. | User interview, Document review | AC-018 |
...
| AC-012 | FR-006 | Lower/mixed-case package-recognized two-letter country input, including a CLDR exceptional region, is stored/returned uppercase; unknown/private/non-alpha-2 first-party country or oversized locality is rejected; only country/locality appear in declaration mutation and response shapes, and events expose no structured location. |
| AC-013 | FR-006, RULE-005 | An independent declaration location with a valid package-recognized country plus extra fields and oversized locality keeps raw source, hydrates country only, and omits extras; non-alpha-2, unknown, or private-use country omits location while the containing declaration remains otherwise servable. |
...
| AC-018 | FR-010 | The pinned `golang.org/x/text/currency` package recognizes currency codes and supplies their standard rounding scales. Price accepts exact `0` and canonical variable-scale values such as USD `1`, `1.2`, `1.23`, rejects USD `1.20`, zero-scale fractions, leading-zero integers, signs, exponents, separators, lowercase/unknown/`XXX`/`XTS` currency, over 12 integer digits, and excess precision. Unsupported independent price is omitted while its product card remains. |
...
| AC-041 | NFR-001 | ADR/schema review confirms all exact bounds, optionality, keys, and the Q17 address AT-URI/CID/path. Tests recompute the vendored record's atproto DAG-CBOR CID, validate representative package-backed country/currency behavior, run lexgen offline from the local external mapping, and repeated generation produces stable output. |
...
| EC-011 | Address country is non-alpha-2, unknown, or private use | Omit the entire location projection but continue serving the eligible containing declaration. | RULE-005 |
...
- Craftsky accepts only country/locality and safe recognized outbound destinations. Country recognition uses the pinned `golang.org/x/text/language` package. Independent extra/unsafe values remain raw and are omitted hydrated.
...
| RISK-012 | Currency rules drift when `golang.org/x/text` is upgraded. | Newly accepted or changed currency precision. | Pin the module version, review dependency upgrades, retain representative canonical validation tests, and omit unsupported independent prices only. |
