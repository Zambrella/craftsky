...
| FR-024 | Functional | Must | Event API shall be `POST /v1/events` plus `GET`, `PUT`, and `DELETE` on `/v1/events/{did}/{rkey}`. Business/event PUT and DELETE require expected CID in `If-Match`, return `409 pds_record_conflict` on mismatch, and successful record responses expose CID. | Prevents lost PDS updates. | User interview, API architecture | AC-037 |
