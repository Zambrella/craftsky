...
Feature: Business declaration presentation
  ...
  Scenario: First-party location input is constrained to the declaration shape
    ...
    When the member submits an unknown, private-use, or non-alpha-2 country, oversized locality, or structured event location
...
Feature: Reproducible business lexicons
  Scenario: Generate code from reviewed schemas
    ...
    And the address DAG-CBOR CID is recomputed and matches
    And representative country and currency validation uses the pinned golang.org/x/text module without network access
...
| UT-006 | FR-006 | AC-012 | Validate and canonicalize first-party declaration location. | Package-recognized lower/mixed/upper alpha-2 country, including a CLDR exceptional region; unknown/private/non-alpha-2 values; locality at and above both bounds; extra mutation fields; event request with structured location. | Recognized country becomes canonical uppercase; unknown/private/invalid country or locality fails; declaration extras and any structured event location are rejected by request shape. | `appview/internal/business/location_test.go` |
| UT-007 | FR-006, RULE-005 | AC-013 | Safely hydrate independent broad declaration addresses. | Recognized country with street/region/postal code and oversized locality; unknown/private/non-alpha-2 country with otherwise valid declaration. | Raw input remains intact; hydration exposes country only in the first case and omits location in the second without dropping the declaration. | `appview/internal/business/location_test.go` |
...
| UT-012 | FR-010 | AC-018 | Validate package-backed currency recognition and canonical variable-scale decimals. | Representative recognized, unknown, `XXX`, and `XTS` codes; `0`; USD `1`, `1.2`, `1.23`, `1.20`; JPY fraction; KWD and four-scale boundaries; 12/13 integer digits; leading zero; signs, exponent, separators. | Recognized uppercase codes use `currency.Standard.Rounding`; unknown/`XXX`/`XTS` codes fail; fractional trailing zeros and noncanonical zero fail. | `appview/internal/business/money_test.go` |
...
| IT-016 | FR-009, NFR-001 | AC-017, AC-041 | Lexicon schemas, 20-product ceiling, cryptographic address pin, package-backed validation, and generated output match reviewed contracts. | Load lexicon JSON, complete CID-named address record, offline mapping, pinned module dependencies, and generated Go package without network. | Recompute address DAG-CBOR CID; validate representative package-backed country/currency behavior; validate 20 products/reject 21; assert schemas/refs/bounds/catalog forms and forbidden fields; run lexicon generation drift twice with network disabled. | CID equals Q17, package-backed validators match the contract, lexgen resolves only the local mapping, contracts match ADR/requirements, and repeated output is stable. | `appview/internal/lexicon/craftsky/business_contract_test.go`, `appview/internal/business/location_test.go`, `appview/internal/business/money_test.go`, `just lexgen-check` |
...
| TD-005 | Address projection corpus | Recognized and unrecognized mixed-case alpha-2 values from `golang.org/x/text/language`; non-alpha-2 forms; bounded/oversized locality; street, region, postal code, coordinates, and unknown extension fields. | UT-006, UT-007, IT-004 |
...
| TD-007 | Money corpus | Exact zero; USD variable-scale and trailing-zero forms; zero-, three-, and four-scale package entries; 12/13 integer digits; signs, exponents, separators, leading zeros; lowercase, unknown, `XXX`, and `XTS` currencies. | UT-012, UT-013, IT-004 |
...
| TD-015 | External contract fixtures | Complete Q17 address record; expected DAG-CBOR CID; pinned `golang.org/x/text` dependency; representative recognized/unrecognized country and currency entries. | AT-013, UT-006, UT-012, IT-016, MAN-001 |

## 8. Manual Checks

| ID | Requirement IDs | Acceptance Criteria | Check | Steps | Expected Result |
|---|---|---|---|---|---|
| MAN-001 | NFR-001 | AC-041 | ADR, schema evolution, external CID, and package-backed validation review. | Review both lexicons, Q17 retrieval/CID recomputation, offline mapping, pinned module use, old-consumer array maxima, generated lexicon types, and ADR against every bound/optional field. | Reviewer confirms durable schemas, external record, package-backed validation, and lexicon generation are explicit, reproducible, additive where required, and approved before implementation merges. |
...
| GAP-002 | Resolved contract; ADR artifact still required for the external address pin and package-backed validation decision. | NFR-001 | AC-041 | Q14/Q15/Q17 fix the package APIs, exact record path, DAG-CBOR CID recomputation, offline mapping, and drift behavior, but project rules still require the ADR before lexicon implementation. | Create the ADR as the first lexicon-design artifact and execute the specified address retrieval/digest/CID verification before adding the vendored external definition. |
