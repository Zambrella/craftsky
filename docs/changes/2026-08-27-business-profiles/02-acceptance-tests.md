...
| UT-022 | FR-024, NFR-003 | AC-037, AC-042 | Parse required If-Match and map conflicts. | Missing, malformed, stale, and current CID; validation errors. | Missing/malformed/stale required precondition maps to camelCase `409 pds_record_conflict` with requestId; current CID proceeds. | `appview/internal/api/business_record_request_test.go` |
...
| IT-003 | FR-003, FR-023 | AC-009, AC-036 | Declaration PUT/DELETE contract and extension-preserving replacement. | Existing PDS declaration with CID, known fields, and unknown top-level extension. | PUT replacement with current If-Match, then DELETE with returned CID. | Exact route full-replaces known fields, preserves extension, exposes CID, and deletion removes only declaration data. | `appview/internal/api/business_profile_test.go` |
