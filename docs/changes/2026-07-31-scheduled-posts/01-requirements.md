...
Decision / implication: Scheduled, Publishing, Retrying, and Needs attention items count transactionally toward the per-DID limit. Published and deleted items do not. At capacity, Schedule for later remains visible but disabled, the composer warns that another post cannot be scheduled while three are retained, and a management link is available; Post now remains available.
...
| FR-006 | Functional | Must | The AppView shall enforce the three-schedule limit transactionally per authenticated DID across concurrent devices/requests; at capacity Flutter shall keep Schedule for later visible but disabled, show a warning that another post cannot be scheduled while three are retained, provide a management link, and leave Post now enabled. | Client checks alone race, while clear capacity UX avoids a dead end. | Prompt / Grilling / UI polish | AC-006, AC-007, AC-026 |
...
| AC-026 | FR-006 | Given all three slots are occupied, Schedule for later remains visible but disabled, the composer warns that another post cannot be scheduled, a Manage scheduled posts link is shown, and Post now remains enabled. |
